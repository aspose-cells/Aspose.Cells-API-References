---
title: export_object_listener 属性
second_title: Aspose.Cells for Python via .NET API 参考文献
description:
type: docs
weight: 280
url: /zh/python-net/aspose.cells/htmlsaveoptions/export_object_listener/
is_root: false
---
## export_object_listener 属性

获取或设置用于导出对象的 ExportObjectListener。

### 评论

注意：此属性现在已过时。
请使用 HtmlSaveOptions.IStreamProvider 属性。
自 2015 年 8 月起 12 个月后，此属性将被删除。
Aspose 对您可能遇到的任何不便深表歉意。
### 定义：
```python
@property
def export_object_listener(self):
    ...
@export_object_listener.setter
def export_object_listener(self, value):
    ...
```

### 也可以看看
* 模块 [aspose.cells](../../)
* 类 [HtmlSaveOptions](/cells/zh/python-net/aspose.cells/htmlsaveoptions)
* 类 [IExportObjectListener](/cells/zh/python-net/aspose.cells/iexportobjectlistener)
