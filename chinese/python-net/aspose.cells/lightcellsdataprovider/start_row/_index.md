...
|范围|类型|描述|
| :- | :- | :- |
| row | [Row](/cells/zh/python-net/aspose.cells/row) |用于实现填充数据的行对象。它的行索引是最新调用[LightCellsDataProvider.next_row()](/cells/zh/python-net/aspose.cells/lightcellsdataprovider/next_row)的返回值。<br/>如果该行已在内部单元格模型中初始化，则将使用现有的行对象。<br/>否则将使用一个临时的 Row 对象来实现填充数据。|
### 评论
