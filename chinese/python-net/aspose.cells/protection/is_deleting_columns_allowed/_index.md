---
title: is_deleting_columns_allowed 属性
second_title: Aspose.Cells for Python via .NET API 参考文献
description:
type: docs
weight: 220
url: /zh/python-net/aspose.cells/protection/is_deleting_columns_allowed/
is_root: false
---
## is_deleting_columns_allowed 属性

表示是否允许在受保护的工作表上删除列。

### 评论

当工作表受到保护时，包含要删除的单元格的列必须解锁，
并且必须启用“选择未锁定的单元格”选项。
### 定义：
```python
@property
def is_deleting_columns_allowed(self):
    ...
@is_deleting_columns_allowed.setter
def is_deleting_columns_allowed(self, value):
    ...
```

### 也可以看看
* 模块 [aspose.cells](../../)
* 类 [Protection](/cells/zh/python-net/aspose.cells/protection)
