---
title: is_inserting_hyperlinks_allowed 属性
second_title: Aspose.Cells for Python via .NET API 参考文献
description:
type: docs
weight: 320
url: /zh/python-net/aspose.cells/protection/is_inserting_hyperlinks_allowed/
is_root: false
---
## is_inserting_hyperlinks_allowed 属性

表示是否允许在受保护的工作表上插入超链接

### 评论

注意：该成员现已过时。
请使用 Protection.Allow Inserting Hyperlink 属性。
自 2010 年 6 月起 12 个月后，此属性将被删除。
Aspose 对您可能遇到的任何不便深表歉意。
### 定义：
```python
@property
def is_inserting_hyperlinks_allowed(self):
    ...
@is_inserting_hyperlinks_allowed.setter
def is_inserting_hyperlinks_allowed(self, value):
    ...
```

### 也可以看看
* 模块 [aspose.cells](../../)
* 类 [Protection](/cells/zh/python-net/aspose.cells/protection)
