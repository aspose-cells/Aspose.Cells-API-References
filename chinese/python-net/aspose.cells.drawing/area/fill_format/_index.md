---
title: fill_format 属性
second_title: Aspose.Cells for Python via .NET API 参考资料
description:
type: docs
weight: 40
url: /zh/python-net/aspose.cells.drawing/area/fill_format/
is_root: false
---
## fill_format 属性

表示包含指定图表或形状的填充格式属性的 [Area.fill_format](/cells/zh/python-net/aspose.cells.drawing/area#fill_format) 对象。
### 定义：
```python
@property
def fill_format(self):
    ...
```

### 也可以看看
* 模块 [aspose.cells.drawing](../../)
* 类 [Area](/cells/zh/python-net/aspose.cells.drawing/area)
* 类 [FillFormat](/cells/zh/python-net/aspose.cells.drawing/fillformat)
