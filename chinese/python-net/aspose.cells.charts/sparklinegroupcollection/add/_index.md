---
title: add方法
second_title: Aspose.Cells for Python via .NET API 参考资料
description:
type: docs
weight: 20
url: /zh/python-net/aspose.cells.charts/sparklinegroupcollection/add/
is_root: false
---
##  add(type, data_range, is_vertical, location_range) {#SparklineType-str-bool-CellArea}
将 [SparklineGroup](/cells/zh/python-net/aspose.cells.charts/sparklinegroup) 项目添加到集合中。


### 返回

[SparklineGroup](/cells/zh/python-net/aspose.cells.charts/sparklinegroup) 对象索引。


```python
def add(self, type, data_range, is_vertical, location_range):
    ...
```


|范围|类型|描述|
| :- | :- | :- |
| type | [SparklineType](/cells/zh/python-net/aspose.cells.charts/sparklinetype) |指定迷你图组的类型。|
| data_range | str |指定迷你图组的数据范围。|
| is_vertical | bool |指定是按行还是按列绘制数据范围内的迷你图。|
| location_range | [CellArea](/cells/zh/python-net/aspose.cells/cellarea) |指定要放置迷你图的位置。|



### 也可以看看
* 模块 [aspose.cells.charts](../../)
* 类 [SparklineGroup](/cells/zh/python-net/aspose.cells.charts/sparklinegroup)
* 类 [SparklineGroupCollection](/cells/zh/python-net/aspose.cells.charts/sparklinegroupcollection)
