---
title: Aspose::Cells::OoxmlSaveOptions::OoxmlSaveOptions constructor
linktitle: OoxmlSaveOptions
second_title: Aspose.Cells for C++ API Reference
description: 'Aspose::Cells::OoxmlSaveOptions::OoxmlSaveOptions constructor. Creates the options for saving office open xml file in C++.'
type: docs
weight: 100
url: /zh/cpp/aspose.cells/ooxmlsaveoptions/ooxmlsaveoptions/
---
## OoxmlSaveOptions::OoxmlSaveOptions() constructor


Creates the options for saving office open xml file.

```cpp
Aspose::Cells::OoxmlSaveOptions::OoxmlSaveOptions()
```

## See Also

* Class [OoxmlSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## OoxmlSaveOptions::OoxmlSaveOptions(SaveFormat) constructor


Creates the options for saving office open xml file.

```cpp
Aspose::Cells::OoxmlSaveOptions::OoxmlSaveOptions(SaveFormat saveFormat)
```


| Parameter | Type | Description |
| --- | --- | --- |
| saveFormat | SaveFormat | The file format. It should be one of following types: [SaveFormat.Xlsx](../../saveformat/), [SaveFormat.Xltx](../../saveformat/), [SaveFormat.Xlam](../../saveformat/), [SaveFormat.Xlsm](../../saveformat/) or [SaveFormat.Xltm](../../saveformat/), otherwise the saved format will be set as [SaveFormat.Xlsx](../../saveformat/) automatically. |

## See Also

* Enum [SaveFormat](../../saveformat/)
* Class [OoxmlSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## OoxmlSaveOptions::OoxmlSaveOptions(OoxmlSaveOptions_Impl*) constructor


Constructs from an implementation object.

```cpp
Aspose::Cells::OoxmlSaveOptions::OoxmlSaveOptions(OoxmlSaveOptions_Impl *impl)
```

## See Also

* Class [Vector](../../vector/)
* Class [OoxmlSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## OoxmlSaveOptions::OoxmlSaveOptions(const OoxmlSaveOptions\&) constructor


Copy constructor.

```cpp
Aspose::Cells::OoxmlSaveOptions::OoxmlSaveOptions(const OoxmlSaveOptions &src)
```

## See Also

* Class [Vector](../../vector/)
* Class [OoxmlSaveOptions](../)
* Class [OoxmlSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## OoxmlSaveOptions::OoxmlSaveOptions(const SaveOptions\&) constructor


Constructs from a parent object.

```cpp
Aspose::Cells::OoxmlSaveOptions::OoxmlSaveOptions(const SaveOptions &src)
```

## See Also

* Class [Vector](../../vector/)
* Class [SaveOptions](../../saveoptions/)
* Class [OoxmlSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
