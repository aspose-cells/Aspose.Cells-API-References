...
description: 初始化Workbookaspose.cells/workbook类.
type: docs
weight: 10
...
默认文件格式类型为 Excel97To2003。如果要创建其他格式文件类型，请调用 Workbook(FileFormatType fileFormatType)。

### 例子

以下代码展示了如何使用 Workbook 构造函数来创建和初始化类的新实例。
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| fileFormatType | FileFormatType | 新的文件格式。 |
...
初始化[`Workbook`](../../workbook)上课并打开一个文件。
...
初始化[`Workbook`](../../workbook)上课并打开一个流。
