---
title: TextOptions
second_title: Справочник по Aspose.Cells для .NET API
description: Представляет параметры текста.
type: docs
weight: 2980
url: /ru/net/aspose.cells.drawing.texts/textoptions/
---
## TextOptions class

Представляет параметры текста.

```csharp
public class TextOptions : Font
```

## Характеристики

| Имя | Описание |
| --- | --- |
| [ArgbColor](../../aspose.cells/font/argbcolor) { get; set; } | Получает и задает цвет с 32-битным значением ARGB. |
| [CapsType](../../aspose.cells/font/capstype) { get; set; } | Получает и устанавливает тип прописных букв. |
| [Charset](../../aspose.cells/font/charset) { get; set; } | Представляет набор символов. |
| [Color](../../aspose.cells/font/color) { get; set; } | Получает или задаетColorшрифта. |
| [DoubleSize](../../aspose.cells/font/doublesize) { get; set; } | Получает и задает двойной размер шрифта. |
| [FarEastName](../../aspose.cells.drawing.texts/textoptions/fareastname) { get; set; } | Получает и задает имя FarEast. |
| [Fill](../../aspose.cells.drawing.texts/textoptions/fill) { get; } | Представляет формат заполнения текста. |
| [IsBold](../../aspose.cells/font/isbold) { get; set; } | Получает или задает значение, указывающее, является ли шрифт полужирным. |
| [IsItalic](../../aspose.cells/font/isitalic) { get; set; } | Получает или задает значение, указывающее, является ли шрифт курсивным. |
| [IsNormalizeHeights](../../aspose.cells/font/isnormalizeheights) { get; set; } | Указывает, выполняется ли нормализация высоты, которая должна применяться к тексту. |
| [IsStrikeout](../../aspose.cells/font/isstrikeout) { get; set; } | Получает или задает значение, указывающее, является ли шрифт одинарным перечеркнутым. |
| [IsSubscript](../../aspose.cells/font/issubscript) { get; set; } | Получает или задает значение, указывающее, является ли шрифт подстрочным. |
| [IsSuperscript](../../aspose.cells/font/issuperscript) { get; set; } | Получает или задает значение, указывающее, является ли шрифт суперсценарным. |
| [Kerning](../../aspose.cells.drawing.texts/textoptions/kerning) { get; set; } | Задает минимальный размер шрифта, при котором будет выполняться кернинг символов для данного прогона текста. |
| [LatinName](../../aspose.cells.drawing.texts/textoptions/latinname) { get; set; } | Получает и устанавливает латинское имя. |
| override [Name](../../aspose.cells.drawing.texts/textoptions/name) { get; set; } | Получает и задает имя фигуры. |
| [Outline](../../aspose.cells.drawing.texts/textoptions/outline) { get; } | Представляет формат структуры текста. |
| [SchemeType](../../aspose.cells/font/schemetype) { get; set; } | Получает и устанавливает тип схемы шрифта. |
| [ScriptOffset](../../aspose.cells/font/scriptoffset) { get; set; } | Получает и устанавливает смещение скрипта в процентах |
| [Shadow](../../aspose.cells.drawing.texts/textoptions/shadow) { get; } | Представляет объект, определяющий эффект тени для элемента диаграммы или фигуры. |
| [Size](../../aspose.cells/font/size) { get; set; } | Получает или задает размер шрифта. |
| [Spacing](../../aspose.cells.drawing.texts/textoptions/spacing) { get; set; } | Задает интервал между символами в текстовом фрагменте. |
| [StrikeType](../../aspose.cells/font/striketype) { get; set; } | Получает тип удара текста. |
| [ThemeColor](../../aspose.cells/font/themecolor) { get; set; } | Получает и устанавливает цвет темы. |
| [Underline](../../aspose.cells/font/underline) { get; set; } | Получает или задает тип подчеркивания шрифта. |
| [UnderlineColor](../../aspose.cells.drawing.texts/textoptions/underlinecolor) { get; set; } | Получает или задает цвет подчеркивания. |

## Методы

| Имя | Описание |
| --- | --- |
| [Equals](../../aspose.cells/font/equals)(Font) | Проверяет, равны ли два шрифта. |
| override [ToString](../../aspose.cells/font/tostring)() | Возвращает строку, представляющую текущий объект Cell. |

### Смотрите также

* class [Font](../../aspose.cells/font)
* пространство имен [Aspose.Cells.Drawing.Texts](../../aspose.cells.drawing.texts)
* сборка [Aspose.Cells](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Cells.dll -->
