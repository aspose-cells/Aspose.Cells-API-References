...
Сбор строковых значений будет полезен только тогда, когда имеется много повторяющихся строковых значений для ячеек, предоставленных этой реализацией. В этой ситуации сбор строк сэкономит много памяти и создаст результирующий файл меньшего размера. Если имеется много строковых значений для предоставленных ячеек с помощью LightCellsDataProvider, но некоторые из них одинаковы, сбор строки будет стоить больше памяти и времени и не будет иметь преимущества для результирующего файла.
