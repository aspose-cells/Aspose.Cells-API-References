...
| Имя | Описание |
| --- | --- |
| [TxtLoadOptions](txtloadoptions#constructor)() | Создает опции для загрузки текстового файла. |
| [TxtLoadOptions](txtloadoptions#constructor_1)(LoadFormat) | Создает опции для загрузки текстового файла. |
...
| [ExtendToNextSheet](../../aspose.cells/txtloadoptions/extendtonextsheet) { get; set; } | Продлевает ли данные на следующий лист, когда строки или столбцы данных превышают лимит. Если это свойство истинно, дополнительные данные будут расширены на следующий лист за текущим (если текущий лист является последним, новый лист будет добавлен к текущей книге). Если это свойство равно false, данные превышают лимит, будут игнорироваться. По умолчанию false; |
...
| [HasTextQualifier](../../aspose.cells/txtloadoptions/hastextqualifier) { get; set; } | Есть ли квалификатор текста для значения ячейки. Значение по умолчанию верно. |
...
| [KeepPrecision](../../aspose.cells/abstracttextloadoptions/keepprecision) { get; set; } | Указывает, не анализируется ли строковое значение, если длина равна 15. |
...
| [TextQualifier](../../aspose.cells/txtloadoptions/textqualifier) { get; set; } | Указывает квалификатор текста для значений ячеек. Квалификатор по умолчанию — '"'. |
...
| [TreatQuotePrefixAsValue](../../aspose.cells/txtloadoptions/treatquoteprefixasvalue) { get; set; } | Указывает, следует ли использовать одинарную кавычку как часть значения одной ячейки. Значение по умолчанию — true. Если оно ложно, ведущая одинарная кавычка будет удалена из значения соответствующей ячейки и[`QuotePrefix`](../style/quoteprefix)будет установлено как true для ячейки. |
