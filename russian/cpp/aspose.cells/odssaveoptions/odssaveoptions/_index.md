---
title: Aspose::Cells::OdsSaveOptions::OdsSaveOptions constructor
linktitle: OdsSaveOptions
second_title: Aspose.Cells for C++ API Reference
description: 'Aspose::Cells::OdsSaveOptions::OdsSaveOptions constructor. Creates the options of saving ods file in C++.'
type: docs
weight: 100
url: /ru/cpp/aspose.cells/odssaveoptions/odssaveoptions/
---
## OdsSaveOptions::OdsSaveOptions() constructor


Creates the options of saving ods file.

```cpp
Aspose::Cells::OdsSaveOptions::OdsSaveOptions()
```

## See Also

* Class [OdsSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## OdsSaveOptions::OdsSaveOptions(SaveFormat) constructor


Creates the options of saving ods file.

```cpp
Aspose::Cells::OdsSaveOptions::OdsSaveOptions(SaveFormat saveFormat)
```


| Parameter | Type | Description |
| --- | --- | --- |
| saveFormat | SaveFormat | The file format. It should be [SaveFormat.Ods](../../saveformat/), [SaveFormat.Ots](../../saveformat/), [SaveFormat.Fods](../../saveformat/) or [SaveFormat.Sxc](../../saveformat/), otherwise the saved format will be set as [SaveFormat.Ods](../../saveformat/) automatically. |

## See Also

* Enum [SaveFormat](../../saveformat/)
* Class [OdsSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## OdsSaveOptions::OdsSaveOptions(OdsSaveOptions_Impl*) constructor


Constructs from an implementation object.

```cpp
Aspose::Cells::OdsSaveOptions::OdsSaveOptions(OdsSaveOptions_Impl *impl)
```

## See Also

* Class [Vector](../../vector/)
* Class [OdsSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## OdsSaveOptions::OdsSaveOptions(const OdsSaveOptions\&) constructor


Copy constructor.

```cpp
Aspose::Cells::OdsSaveOptions::OdsSaveOptions(const OdsSaveOptions &src)
```

## See Also

* Class [Vector](../../vector/)
* Class [OdsSaveOptions](../)
* Class [OdsSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## OdsSaveOptions::OdsSaveOptions(const SaveOptions\&) constructor


Constructs from a parent object.

```cpp
Aspose::Cells::OdsSaveOptions::OdsSaveOptions(const SaveOptions &src)
```

## See Also

* Class [Vector](../../vector/)
* Class [SaveOptions](../../saveoptions/)
* Class [OdsSaveOptions](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
