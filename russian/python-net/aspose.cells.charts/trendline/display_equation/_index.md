---
title: display_equation недвижимость
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 120
url: /ru/python-net/aspose.cells.charts/trendline/display_equation/
is_root: false
---
##  display_equation недвижимость

Указывает, отображается ли уравнение для линии тренда на диаграмме (в той же метке данных, что и значение R-квадрата).
###  Определение:
```python
@property
def display_equation(self):
    ...
@display_equation.setter
def display_equation(self, value):
    ...
```

###  Смотрите также
* модуль [aspose.cells.charts](../../)
* класс [Trendline](/cells/ru/python-net/aspose.cells.charts/trendline)
