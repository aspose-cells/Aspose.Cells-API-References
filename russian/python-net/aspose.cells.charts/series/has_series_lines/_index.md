---
title: has_series_lines недвижимость
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 240
url: /ru/python-net/aspose.cells.charts/series/has_series_lines/
is_root: false
---
##  has_series_lines недвижимость

Значение true, если столбчатая диаграмма с накоплением или линейчатая диаграмма содержит ряды линий или
 если круговая диаграмма или гистограмма имеет соединительные линии между двумя разделами.
Применяется только к столбчатым диаграммам с накоплением, линейчатым диаграммам, круговым диаграммам или линейчатым круговым диаграммам.
###  Определение:
```python
@property
def has_series_lines(self):
    ...
@has_series_lines.setter
def has_series_lines(self, value):
    ...
```

###  Смотрите также
* модуль [aspose.cells.charts](../../)
* класс [Series](/cells/ru/python-net/aspose.cells.charts/series)
