---
title: export_object_listener недвижимость
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 280
url: /ru/python-net/aspose.cells/htmlsaveoptions/export_object_listener/
is_root: false
---
##  export_object_listener недвижимость

Получает или задает ExportObjectListener для экспорта объектов.

###  Примечания

 ПРИМЕЧАНИЕ. Это свойство устарело. Вместо,
используйте свойство HtmlSaveOptions.IStreamProvider.
 Это свойство будет удалено через 12 месяцев, начиная с августа 2015 года.
Aspose приносит извинения за возможные неудобства.
###  Определение:
```python
@property
def export_object_listener(self):
    ...
@export_object_listener.setter
def export_object_listener(self, value):
    ...
```

###  Смотрите также
* модуль [aspose.cells](../../)
* класс [HtmlSaveOptions](/cells/ru/python-net/aspose.cells/htmlsaveoptions)
* класс [IExportObjectListener](/cells/ru/python-net/aspose.cells/iexportobjectlistener)
