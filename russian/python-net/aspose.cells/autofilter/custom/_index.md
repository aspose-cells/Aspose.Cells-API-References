---
title: custom метод
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 70
url: /ru/python-net/aspose.cells/autofilter/custom/
is_root: false
---
##  custom(field_index, operator_type1, criteria1) {#int-FilterOperatorType-any}
Фильтрует список по критерию custom.



```python
def custom(self, field_index, operator_type1, criteria1):
    ...
```


| Параметры| Тип| Описание|
| :- | :- | :- |
| field_index | int | Целочисленное смещение поля, на котором вы хотите основывать фильтр.<br/> (слева в списке; крайнее левое поле — поле 0).|
| operator_type1 | [FilterOperatorType](/cells/ru/python-net/aspose.cells/filteroperatortype) | Тип оператора фильтра|
| criteria1 | any | Пользовательские критерии|


##  custom(field_index, operator_type1, criteria1, is_and, operator_type2, criteria2) {#int-FilterOperatorType-any-bool-FilterOperatorType-any}
Фильтрует список с критериями custom.



```python
def custom(self, field_index, operator_type1, criteria1, is_and, operator_type2, criteria2):
    ...
```


| Параметры| Тип| Описание|
| :- | :- | :- |
| field_index | int | Целочисленное смещение поля, на котором вы хотите основывать фильтр.<br/> (слева в списке; крайнее левое поле — поле 0).|
| operator_type1 | [FilterOperatorType](/cells/ru/python-net/aspose.cells/filteroperatortype) | Тип оператора фильтра|
| criteria1 | any | Пользовательские критерии|
| is_and | bool |  |
| operator_type2 | [FilterOperatorType](/cells/ru/python-net/aspose.cells/filteroperatortype) | Тип оператора фильтра|
| criteria2 | any | Пользовательские критерии|



###  Смотрите также
* модуль [aspose.cells](../../)
* класс [AutoFilter](/cells/ru/python-net/aspose.cells/autofilter)
