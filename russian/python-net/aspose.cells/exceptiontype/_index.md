---
title: ExceptionType перечисление
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 2020
url: /ru/python-net/aspose.cells/exceptiontype/
is_root: false
---
##  ExceptionType перечисление
Представляет код пользовательского типа исключения.



Тип ExceptionType предоставляет следующие члены:

###  Поля
| Поле| Описание|
| :- | :- |
| CHART | Неверная настройка диаграммы.|
| DATA_TYPE | Недопустимая настройка типа данных.|
| DATA_VALIDATION | Недопустимая настройка проверки данных.|
| CONDITIONAL_FORMATTING | Недопустимая настройка проверки данных.|
| FILE_FORMAT | Недопустимый формат файла.|
| FORMULA | Неверная формула.|
| INVALID_DATA | Неверные данные.|
| INVALID_OPERATOR | Неверный оператор.|
| INCORRECT_PASSWORD | Неверный пароль.|
| LICENSE | Ошибки, связанные с лицензией.|
| LIMITATION |Ошибка ограничения MS Excel.|
| PAGE_SETUP | Неверный параметр настройки страницы.|
| PIVOT_TABLE | Недопустимая настройка сводной таблицы.|
| SHAPE | Недопустимая настройка объекта рисования.|
| SPARKLINE | Недопустимая настройка объекта спарклайна.|
| SHEET_NAME | Недопустимое имя рабочего листа.|
| SHEET_TYPE | Недопустимый тип рабочего листа.|
| INTERRUPTED | Процесс прерывается.|
| IO | Файл недействителен.|
| PERMISSION | Требуется разрешение, чтобы открыть этот файл.|
| UNSUPPORTED_FEATURE | Неподдерживаемая функция.|
| UNSUPPORTED_STREAM | Неподдерживаемый поток для открытия.|
| UNDISCLOSED_INFORMATION | Файлы содержат некоторую нераскрытую информацию.|
| FILE_CORRUPTED | Содержимое файла повреждено.|



###  Смотрите также
* модуль [aspose.cells](..)
