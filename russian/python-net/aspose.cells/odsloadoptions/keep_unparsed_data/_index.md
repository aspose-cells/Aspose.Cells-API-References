---
title: keep_unparsed_data недвижимость
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 130
url: /ru/python-net/aspose.cells/odsloadoptions/keep_unparsed_data/
is_root: false
---
##  keep_unparsed_data недвижимость

Сохранять ли непроанализированные данные в памяти для рабочей книги, когда она загружается из файла шаблона. По умолчанию — true.

###  Примечания

Для сценариев, в которых пользователю нужно только прочитать некоторое содержимое из файла шаблона и не нужно сохранять книгу обратно,
установка этого свойства как false может улучшить производительность, особенно при использовании его вместе с каким-либо LoadFilter,
###  Определение:
```python
@property
def keep_unparsed_data(self):
    ...
@keep_unparsed_data.setter
def keep_unparsed_data(self, value):
    ...
```

###  Смотрите также
* модуль [aspose.cells](../../)
* класс [OdsLoadOptions](/cells/ru/python-net/aspose.cells/odsloadoptions)
