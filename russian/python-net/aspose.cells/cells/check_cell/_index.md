---
title: check_cell метод
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 60
url: /ru/python-net/aspose.cells/cells/check_cell/
is_root: false
---
##  check_cell(row, column) {#int-int}
Получает элемент [Cell](/cells/ru/python-net/aspose.cells/cell) или значение NULL по указанному индексу строки ячейки и индексу столбца.


###  Возвращает

Возвращает объект Cell, если объект Cell существует.
Возвращает null, если ячейка не существует.


```python
def check_cell(self, row, column):
    ...
```


| Параметры| Тип| Описание|
| :- | :- | :- |
| row | int | Индекс строки|
| column | int | Индекс столбца|



###  Смотрите также
* модуль [aspose.cells](../../)
* класс [Cell](/cells/ru/python-net/aspose.cells/cell)
* класс [Cells](/cells/ru/python-net/aspose.cells/cells)
