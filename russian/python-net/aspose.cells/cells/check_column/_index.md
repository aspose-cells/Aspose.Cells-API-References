---
title: check_column метод
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 70
url: /ru/python-net/aspose.cells/cells/check_column/
is_root: false
---
##  check_column(column_index) {#int}
Получает элемент [Column](/cells/ru/python-net/aspose.cells/column) или null по указанному индексу столбца.


###  Возвращает

Объект Столбец.


```python
def check_column(self, column_index):
    ...
```


| Параметр| Тип| Описание|
| :- | :- | :- |
| column_index | int | Индекс столбца.|



###  Смотрите также
* модуль [aspose.cells](../../)
* класс [Cells](/cells/ru/python-net/aspose.cells/cells)
* класс [Column](/cells/ru/python-net/aspose.cells/column)
