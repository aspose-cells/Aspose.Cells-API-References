---
title: InsertOptions класс
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 940
url: /ru/python-net/aspose.cells/insertoptions/
is_root: false
---
##  InsertOptions класс
Представляет варианты вставки.



Тип InsertOptions предоставляет следующие члены:

###  Конструкторы
| Конструктор| Описание|
| :- | :- |
| [InsertOptions()](/cells/ru/python-net/aspose.cells/insertoptions/__init__/#) | Создает новый экземпляр InsertOptions|


###  Характеристики
| Свойство| Описание|
| :- | :- |
| [copy_format_type](/cells/ru/python-net/aspose.cells/insertoptions/copy_format_type) |  |
| [update_reference](/cells/ru/python-net/aspose.cells/insertoptions/update_reference) |Указывает, будут ли обновлены ссылки на других рабочих листах.|



###  Смотрите также
* модуль [aspose.cells](..)
