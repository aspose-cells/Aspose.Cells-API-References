---
title: add_blank_line_between_rows недвижимость
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 30
url: /ru/python-net/aspose.cells.saving/sqlscriptsaveoptions/add_blank_line_between_rows/
is_root: false
---
##  add_blank_line_between_rows недвижимость

Вставьте пустую строку между каждыми данными.

###  Примечания

Если [SqlScriptSaveOptions.separator](/cells/ru/python-net/aspose.cells.saving/sqlscriptsaveoptions#separator) равно '\n' , лучше установить для этого свойства значение true, чтобы улучшить читаемость.
###  Определение:
```python
@property
def add_blank_line_between_rows(self):
    ...
@add_blank_line_between_rows.setter
def add_blank_line_between_rows(self, value):
    ...
```

###  Смотрите также
* модуль [aspose.cells.saving](../../)
* класс [SqlScriptSaveOptions](/cells/ru/python-net/aspose.cells.saving/sqlscriptsaveoptions)
