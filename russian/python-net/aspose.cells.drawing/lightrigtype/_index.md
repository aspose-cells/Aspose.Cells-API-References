---
title: LightRigType перечисление
second_title: Aspose.Cells for Python via .NET API
description:
type: docs
weight: 920
url: /ru/python-net/aspose.cells.drawing/lightrigtype/
is_root: false
---
##  LightRigType перечисление
Представляет предустановленное право света, которое можно применить к фигуре.



Тип LightRigType предоставляет следующие члены:

###  Поля
| Поле| Описание|
| :- | :- |
| BALANCED | Сбалансированный|
| BRIGHT_ROOM | Светлая комната|
| CHILLY | Холодно|
| CONTRASTING | контраст|
| FLAT | Плоский|
| FLOOD | Наводнение|
| FREEZING | Замораживание|
| GLOW | Светиться|
| HARSH | Жесткий|
| LEGACY_FLAT1 | НаследиеКвартира1|
| LEGACY_FLAT2 | НаследиеКвартира2|
| LEGACY_FLAT3 | НаследиеКвартира3|
| LEGACY_FLAT4 | LegacyFlat4|
| LEGACY_HARSH1 | НаследиеHarsh1|
| LEGACY_HARSH2 | НаследиеHarsh2|
| LEGACY_HARSH3 | НаследиеHarsh3|
| LEGACY_HARSH4 | НаследиеHarsh4|
| LEGACY_NORMAL1 | НаследиеОбычный1|
| LEGACY_NORMAL2 | НаследиеОбычный2|
| LEGACY_NORMAL3 | НаследиеОбычный3|
| LEGACY_NORMAL4 | НаследиеОбычный4|
| MORNING | Утро|
| SOFT | Мягкий|
| SUNRISE | Восход|
| SUNSET | Закат|
| THREE_POINT | Три точки|
| TWO_POINT | Две точки|
| NONE | Нет легкой установки.|



###  Смотрите также
* модуль [aspose.cells.drawing](..)
