...
type: docs
weight: 2800
url: /fr/cpp/aspose.cells.drawing/line/
---
## Line class
...
//Appliquer un style de ligne pointillée sur les lignes d'une NSeries
chart.GetNSeries().Get(0).GetBorder().SetStyle(LineType::Dot);
chart.GetNSeries().Get(0).GetBorder().SetColor(Color{ 0xff, 0xff, 0, 0 });
//Appliquer un style de repère triangulaire sur les repères de données d'une NSeries
chart.GetNSeries().Get(0).GetMarker().SetMarkerStyle(ChartMarkerType::Triangle);
//Définir le poids de toutes les lignes d'une NSeries à moyen
