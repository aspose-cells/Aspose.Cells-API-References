---
title: add méthode
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 20
url: /fr/python-net/aspose.cells.tables/listobjectcollection/add/
is_root: false
---
##  add(start_cell, end_cell, has_headers) {#str-str-bool}
Ajoute un ListObject à la feuille de calcul.


###  Retour

L'index du nouveau ListObject


```python
def add(self, start_cell, end_cell, has_headers):
    ...
```


| Paramètres| Taper| Description|
| :- | :- | :- |
| start_cell | str | Cellule de début de la plage de liste.|
| end_cell | str | Cellule de fin de la plage de la liste.|
| has_headers | bool | Si la plage a des en-têtes.|


##  add(start_row, start_column, end_row, end_column, has_headers) {#int-int-int-int-bool}
Ajoute un ListObject à la feuille de calcul.


###  Retour

L'index du nouveau ListObject


```python
def add(self, start_row, start_column, end_row, end_column, has_headers):
    ...
```


| Paramètres| Taper| Description|
| :- | :- | :- |
| start_row | int | La ligne de début de la plage de liste.|
| start_column | int | La ligne de début de la plage de liste.|
| end_row | int | La ligne de début de la plage de liste.|
| end_column | int | La ligne de début de la plage de liste.|
| has_headers | bool | Si la plage a des en-têtes.|



###  Voir également
* module [aspose.cells.tables](../../)
* classe [ListObjectCollection](/cells/fr/python-net/aspose.cells.tables/listobjectcollection)
