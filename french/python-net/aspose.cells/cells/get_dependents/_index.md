---
title: get_dependents méthode
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 380
url: /fr/python-net/aspose.cells/cells/get_dependents/
is_root: false
---
##  get_dependents(is_all, row, column) {#bool-int-int}
Obtenez toutes les cellules qui font référence à la cellule spécifique.


###  Retour




```python
def get_dependents(self, is_all, row, column):
    ...
```


| Paramètre| Taper| Description|
| :- | :- | :- |
| is_all | bool | Indique si vérifier d'autres feuilles de calcul|
| row | int | L'index de ligne.|
| column | int | L'indice de colonne.|



###  Voir également
* module [aspose.cells](../../)
* classe [Cells](/cells/fr/python-net/aspose.cells/cells)
