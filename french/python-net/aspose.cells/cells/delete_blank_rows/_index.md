---
title: delete_blank_rows méthode
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 210
url: /fr/python-net/aspose.cells/cells/delete_blank_rows/
is_root: false
---
##  delete_blank_rows() {#}
Supprimez toutes les lignes vides qui ne contiennent aucune donnée.



```python
def delete_blank_rows(self):
    ...
```




##  delete_blank_rows(options) {#DeleteOptions}
Supprimez toutes les lignes vides qui ne contiennent aucune donnée.



```python
def delete_blank_rows(self, options):
    ...
```


| Paramètre| Taper| Description|
| :- | :- | :- |
| options | [DeleteOptions](/cells/fr/python-net/aspose.cells/deleteoptions) | Les options de suppression de plage.|



###  Voir également
* module [aspose.cells](../../)
* classe [Cells](/cells/fr/python-net/aspose.cells/cells)
