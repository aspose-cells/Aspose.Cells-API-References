---
title: set_picture méthode
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 220
url: /fr/python-net/aspose.cells/pagesetup/set_picture/
is_root: false
---
##  set_picture(is_first, is_even, is_header, section, image_data) {#bool-bool-bool-int-bytes}
Définit une image dans l'en-tête/le pied de page d'une feuille de calcul.


###  Retour

Renvoie l'objet [Picture](/cells/fr/python-net/aspose.cells.drawing/picture).


```python
def set_picture(self, is_first, is_even, is_header, section, image_data):
    ...
```


| Paramètre| Taper| Description|
| :- | :- | :- |
| is_first | bool | Indique si l'image de l'en-tête/pied de page de la première page est définie.|
| is_even | bool | Indique si le réglage de l'image de l'en-tête/pied de page pair.|
| is_header | bool | Indique si le réglage de l'image d'en-tête/pied de page.|
| section | int | 0 : Section gauche, 1 : Section centrale, 2 : Section droite.|
| image_data | bytes | Données d'image.|



###  Voir également
* module [aspose.cells](../../)
* classe [PageSetup](/cells/fr/python-net/aspose.cells/pagesetup)
* classe [Picture](/cells/fr/python-net/aspose.cells.drawing/picture)
