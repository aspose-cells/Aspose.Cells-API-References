---
title: get_cell_or_null méthode
second_title: Aspose.Cells for Python via .NET API Références
description:
type: docs
weight: 80
url: /fr/python-net/aspose.cells/range/get_cell_or_null/
is_root: false
---
##  get_cell_or_null(row_offset, column_offset) {#int-int}
Obtient [Cell](/cells/fr/python-net/aspose.cells/cell) objet ou null dans cette plage.


###  Retour

[Cell](/cells/fr/python-net/aspose.cells/cell) objet.


```python
def get_cell_or_null(self, row_offset, column_offset):
    ...
```


| Paramètre| Taper| Description|
| :- | :- | :- |
| row_offset | int | Décalage de ligne dans cette plage, basé sur zéro.|
| column_offset | int |Décalage de colonne dans cette plage, basé sur zéro.|



###  Voir également
* module [aspose.cells](../../)
* classe [Cell](/cells/fr/python-net/aspose.cells/cell)
* classe [Range](/cells/fr/python-net/aspose.cells/range)
