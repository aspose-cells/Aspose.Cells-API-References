---
title: calculate_on_save mülk
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 40
url: /tr/python-net/aspose.cells/formulasettings/calculate_on_save/
is_root: false
---
##  calculate_on_save mülk

El ile hesaplama modundayken, belgeyi kaydetmeden önce çalışma kitabının yeniden hesaplanıp hesaplanmayacağını belirtir.

###  Notlar

Bu özellik, yalnızca ayarları sonuçtaki elektronik tablo dosyasına kaydetmek içindir
böylece diğer uygulamalar (ms excel gibi) ortaya çıkan dosyayı yüklerken ve değiştirirken buna göre hareket edebilir.
Çoğu kullanıcının uygulamalarındaki performans değerlendirmesi için, çalışma kitabındaki hiçbir formülü otomatik olarak hesaplamayız,
Bu özellik için hangi değer ayarlanmış olursa olsun.
###  Tanım:
```python
@property
def calculate_on_save(self):
    ...
@calculate_on_save.setter
def calculate_on_save(self, value):
    ...
```

###  Ayrıca bakınız
* modül [aspose.cells](../../)
* sınıf [FormulaSettings](/cells/tr/python-net/aspose.cells/formulasettings)
