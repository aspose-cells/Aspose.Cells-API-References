---
title: text_to_columns yöntemi
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 930
url: /tr/python-net/aspose.cells/cells/text_to_columns/
is_root: false
---
##  text_to_columns(row, column, total_rows, options) {#int-int-int-TxtLoadOptions}
Sütunlardaki metni sütunlara böler.



```python
def text_to_columns(self, row, column, total_rows, options):
    ...
```


| parametreler| Tip| Tanım|
| :- | :- | :- |
| row | int | Satır dizini.|
| column | int | Sütun dizini.|
| total_rows | int | Satır sayısı.|
| options | [TxtLoadOptions](/cells/tr/python-net/aspose.cells/txtloadoptions) | Bölme seçenekleri.|



###  Ayrıca bakınız
* modül [aspose.cells](../../)
* sınıf [Cells](/cells/tr/python-net/aspose.cells/cells)
