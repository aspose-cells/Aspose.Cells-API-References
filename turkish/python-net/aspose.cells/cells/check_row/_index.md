---
title: check_row yöntemi
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 80
url: /tr/python-net/aspose.cells/cells/check_row/
is_root: false
---
##  check_row(row) {#int}
[Row](/cells/tr/python-net/aspose.cells/row) öğesini veya belirtilen hücre satırı dizinini alır.


###  İadeler

Row nesnesi varsa, Row nesnesini döndürür, aksi takdirde null değerini döndürür.


```python
def check_row(self, row):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| row | int | Satır dizini|



###  Ayrıca bakınız
* modül [aspose.cells](../../)
* sınıf [Cells](/cells/tr/python-net/aspose.cells/cells)
* sınıf [Row](/cells/tr/python-net/aspose.cells/row)
