---
title: insert_cut_cells yöntemi
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 700
url: /tr/python-net/aspose.cells/cells/insert_cut_cells/
is_root: false
---
##  insert_cut_cells(cut_range, row, column, shift_type) {#Range-int-int-ShiftType}
Kesme aralığını girin.



```python
def insert_cut_cells(self, cut_range, row, column, shift_type):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| cut_range | [Range](/cells/tr/python-net/aspose.cells/range) | Kesim aralığı.|
| row | int | Satır.|
| column | int | Sütun.|
| shift_type | [ShiftType](/cells/tr/python-net/aspose.cells/shifttype) | Vardiya türü.|



###  Ayrıca bakınız
* modül [aspose.cells](../../)
* sınıf [Cells](/cells/tr/python-net/aspose.cells/cells)
