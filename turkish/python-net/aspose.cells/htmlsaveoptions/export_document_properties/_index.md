---
title: export_document_properties mülk
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 200
url: /tr/python-net/aspose.cells/htmlsaveoptions/export_document_properties/
is_root: false
---
##  export_document_properties mülk

 Belge özelliklerinin dışa aktarılıp aktarılmadığını belirtir. Varsayılan değer true'dur. İçe aktarmak istiyorsanız
html veya mht dosyasını excel'e gönderirken, lütfen varsayılan değeri koruyun.
###  Tanım:
```python
@property
def export_document_properties(self):
    ...
@export_document_properties.setter
def export_document_properties(self, value):
    ...
```

###  Ayrıca bakınız
* modül [aspose.cells](../../)
* sınıf [HtmlSaveOptions](/cells/tr/python-net/aspose.cells/htmlsaveoptions)
