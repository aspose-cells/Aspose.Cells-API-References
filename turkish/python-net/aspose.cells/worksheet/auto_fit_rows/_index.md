---
title: auto_fit_rows yöntemi
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 70
url: /tr/python-net/aspose.cells/worksheet/auto_fit_rows/
is_root: false
---
##  auto_fit_rows() {#}
Bu çalışma sayfasındaki tüm satırları otomatik sığdırır.



```python
def auto_fit_rows(self):
    ...
```




##  auto_fit_rows(only_auto) {#bool}
Bu çalışma sayfasındaki tüm satırları otomatik sığdırır.



```python
def auto_fit_rows(self, only_auto):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| only_auto | bool |Doğru, yalnızca satır yüksekliği özelleştirilmediğinde satır yüksekliğini otomatik olarak sığdırır.|


##  auto_fit_rows(options) {#AutoFitterOptions}
Bu çalışma sayfasındaki tüm satırları otomatik sığdırır.



```python
def auto_fit_rows(self, options):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| options | [AutoFitterOptions](/cells/tr/python-net/aspose.cells/autofitteroptions) | Otomatik tesisatçı seçenekleri|


##  auto_fit_rows(start_row, end_row) {#int-int}
Bir aralıktaki satır yüksekliğini otomatik sığdırır.



```python
def auto_fit_rows(self, start_row, end_row):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| start_row | int | Satır dizini başlat.|
| end_row | int | Son satır dizini.|


##  auto_fit_rows(start_row, end_row, options) {#int-int-AutoFitterOptions}
Bir aralıktaki satır yüksekliğini otomatik sığdırır.



```python
def auto_fit_rows(self, start_row, end_row, options):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| start_row | int | Satır dizini başlat.|
| end_row | int | Son satır dizini.|
| options | [AutoFitterOptions](/cells/tr/python-net/aspose.cells/autofitteroptions) | Otomatik tesisatçı seçenekleri.|



###  Ayrıca bakınız
* modül [aspose.cells](../../)
* sınıf [Worksheet](/cells/tr/python-net/aspose.cells/worksheet)
