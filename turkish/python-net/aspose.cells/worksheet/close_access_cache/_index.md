---
title: close_access_cache yöntemi
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 100
url: /tr/python-net/aspose.cells/worksheet/close_access_cache/
is_root: false
---
##  close_access_cache(opts) {#AccessCacheOptions}
Bu çalışma sayfasındaki verilere erişmek için önbellekleri kullanan oturumu kapatır.



```python
def close_access_cache(self, opts):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| opts | [AccessCacheOptions](/cells/tr/python-net/aspose.cells/accesscacheoptions) | veri erişim seçenekleri|



###  Ayrıca bakınız
* modül [aspose.cells](../../)
* sınıf [Worksheet](/cells/tr/python-net/aspose.cells/worksheet)
