---
title: add_calculated_field yöntemi
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 20
url: /tr/python-net/aspose.cells.pivot/pivottable/add_calculated_field/
is_root: false
---
##  add_calculated_field(name, formula) {#str-str}
Pivot alana hesaplanmış bir alan ekler ve bunu veri alanına sürükler.



```python
def add_calculated_field(self, name, formula):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| name | str | Hesaplanan alanın adı|
| formula | str | Hesaplanan alanın formülü.|


##  add_calculated_field(name, formula, drag_to_data_area) {#str-str-bool}
Pivot alana hesaplanmış bir alan ekler.



```python
def add_calculated_field(self, name, formula, drag_to_data_area):
    ...
```


| Parametre| Tip| Tanım|
| :- | :- | :- |
| name | str | Hesaplanan alanın adı|
| formula | str | Hesaplanan alanın formülü.|
| drag_to_data_area | bool | Doğru, bu alanı hemen veri alanına sürükleyin|



###  Ayrıca bakınız
* modül [aspose.cells.pivot](../../)
* sınıf [PivotTable](/cells/tr/python-net/aspose.cells.pivot/pivottable)
