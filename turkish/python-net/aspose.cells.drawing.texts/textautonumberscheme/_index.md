---
title: TextAutonumberScheme numaralandırma
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 160
url: /tr/python-net/aspose.cells.drawing.texts/textautonumberscheme/
is_root: false
---
##  TextAutonumberScheme numaralandırma
Tüm otomatik sayı düzenini temsil eder.



TextAutonumberScheme türü aşağıdaki üyeleri gösterir:

###  alanlar
| Alan| Tanım|
| :- | :- |
| NONE |  |
| ALPHA_LC_PAREN_BOTH | (a), (b), (c), …|
| ALPHA_LC_PAREN_R | a), b), c), …|
| ALPHA_LC_PERIOD | a., b., c., …|
| ALPHA_UC_PAREN_BOTH | (A), (B), (C), …|
| ALPHA_UC_PAREN_R | A), B), C), …|
| ALPHA_UC_PERIOD | A., B., C., …|
| ARABIC_1_MINUS | ANSI eksi sembolü ile Bidi Arapça 1 (AraAlpha)|
| ARABIC_2_MINUS | ANSI eksi sembolü ile Bidi Arapça 2 (AraAbjad)|
| ARABIC_DB_PERIOD | Dbl-byte Arapça sayılar, çift baytlık nokta ile|
| ARABIC_DB_PLAIN | Dbl bayt Arapça sayılar|
| ARABIC_PAREN_BOTH | (1), (2), (3), … |
| ARABIC_PAREN_R | 1), 2), 3), … |
| ARABIC_PERIOD | 1., 2., 3., … |
| ARABIC_PLAIN | 1, 2, 3, … |
| CIRCLE_NUM_DB_PLAIN | Dbl-byte daire numaraları (1-10 daire[0x2460-], 11-arapça sayılar)|
| CIRCLE_NUM_WD_BLACK_PLAIN | Wingdings siyah daire numaraları|
| CIRCLE_NUM_WD_WHITE_PLAIN | Wingdings beyaz daire sayıları (0-10 daire[0x0080-],11- arapça sayılar)|
| EA_1_CHS_PERIOD | EA: Tek baytlık dönemle basitleştirilmiş Çince|
| EA_1_CHS_PLAIN | EA: Basitleştirilmiş Çince (TypeA 1-99, TypeC 100-)|
| EA_1_CHT_PERIOD | EA: Tek baytlık nokta ile Geleneksel Çince|
| EA_1_CHT_PLAIN |EA: Geleneksel Çince (TypeA 1-19, TypeC 20-)|
| EA_1_JPN_CHS_DB_PERIOD | EA: Çift bayt noktalı Japonca|
| EA_1_JPN_KOR_PERIOD | EA: Japonca/Korece w/ tek baytlık nokta|
| EA_1_JPN_KOR_PLAIN | EA: Japonca/Korece (TypeC 1-)|
| HEBREW_2_MINUS | ANSI eksi sembolü ile Bidi İbranice 2|
| HINDI_ALPHA_1_PERIOD | Hint alfabesi dönemi - ünsüzler|
| HINDI_ALPHA_PERIOD | Hint alfabesi dönemi - ünlüler|
| HINDI_NUM_PAREN_R | Hintçe sayısal parantezler - sağ|
| HINDI_NUM_PERIOD | Hintçe sayısal dönem|
| ROMAN_LC_PAREN_BOTH | (i), (ii), (iii), …|
| ROMAN_LC_PAREN_R | i), ii), iii), …|
| ROMAN_LC_PERIOD | i., ii., iii., …|
| ROMAN_UC_PAREN_BOTH | (I), (II), (III), …|
| ROMAN_UC_PAREN_R | I), II), III), …|
| ROMAN_UC_PERIOD | I., II., III., …|
| THAI_ALPHA_PAREN_BOTH | Tay alfabesi parantezleri - her ikisi de|
| THAI_ALPHA_PAREN_R | Tay alfabesi parantezleri - sağ|
| THAI_ALPHA_PERIOD | Tay alfabesi dönemi|
| THAI_NUM_PAREN_BOTH | Tayca sayısal parantezler - her ikisi de|
| THAI_NUM_PAREN_R | Tayca sayısal parantezler - sağ|
| THAI_NUM_PERIOD | Tay sayısal dönemi|



###  Ayrıca bakınız
* modül [aspose.cells.drawing.texts](..)
