---
title: copy_comments_in_range yöntemi
second_title: Aspose.Cells for Python via .NET API Referanslar
description:
type: docs
weight: 370
url: /tr/python-net/aspose.cells.drawing/shapecollection/copy_comments_in_range/
is_root: false
---
##  copy_comments_in_range(shapes, ca, dest_row, dest_column) {#ShapeCollection-CellArea-int-int}
Aralıktaki tüm yorumları kopyalayın.



```python
def copy_comments_in_range(self, shapes, ca, dest_row, dest_column):
    ...
```


| parametreler| Tip| Tanım|
| :- | :- | :- |
| shapes | [ShapeCollection](/cells/tr/python-net/aspose.cells.drawing/shapecollection) | Kaynak şekiller.|
| ca | [CellArea](/cells/tr/python-net/aspose.cells/cellarea) | Kaynak aralığı.|
| dest_row | int | Hedef aralık başlangıç satırı.|
| dest_column | int | Hedef aralık başlangıç sütunu.|



###  Ayrıca bakınız
* modül [aspose.cells.drawing](../../)
* sınıf [ShapeCollection](/cells/tr/python-net/aspose.cells.drawing/shapecollection)
