...
```javascript
const { Workbook, XlsSaveOptions } = AsposeCells;
...
var uint8Array = workbook.save(saveOptions);
```
## Constructors
...
| [replace(boolean, VObject)](#replace-boolean-vobject-)| Replaces cells' values with new data. |
| [replace(number, VObject)](#replace-number-vobject-)| Replaces cells' values with new data. |
...
### replace(boolean, VObject) {#replace-boolean-vobject-}

Replaces cells' values with new data.

```javascript
replace(boolValue: boolean, newValue: VObject) : number;
```
...
| newValue | VObject | New value. Can be string, integer, double or DateTime value. |

### replace(number, VObject) {#replace-number-vobject-}

Replaces cells' values with new data.

```javascript
replace(intValue: number, newValue: VObject) : number;
```
