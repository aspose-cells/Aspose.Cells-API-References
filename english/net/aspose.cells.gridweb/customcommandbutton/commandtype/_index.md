---
title: CustomCommandButton.CommandType
second_title: Aspose.Cells for .NET API Reference
description: CustomCommandButton property. Gets or sets the rendering type of the command. The type can be command button or context menu item
type: docs
url: /net/aspose.cells.gridweb/customcommandbutton/commandtype/
---
## CustomCommandButton.CommandType property

Gets or sets the rendering type of the command. The type can be command button or context menu item.

```csharp
public CustomCommandButtonType CommandType { get; set; }
```

### See Also

* enum [CustomCommandButtonType](../../customcommandbuttontype/)
* class [CustomCommandButton](../)
* namespace [Aspose.Cells.GridWeb](../../../aspose.cells.gridweb/)
* assembly [Aspose.Cells.GridWeb](../../../)


