---
title: IGridJsService.DetailStreamJson
second_title: Aspose.Cells for .NET API Reference
description: IGridJsService method. Write the JSON string for the Workbook to the stream
type: docs
url: /net/aspose.cells.gridjs/igridjsservice/detailstreamjson/
---
## DetailStreamJson(Stream, string) {#detailstreamjson_1}

Write the JSON string for the Workbook to the stream

```csharp
public void DetailStreamJson(Stream stream, string filePath)
```

| Parameter | Type | Description |
| --- | --- | --- |
| stream | Stream | The stream that will be written |
| filePath | String | The file path |

### See Also

* interface [IGridJsService](../)
* namespace [Aspose.Cells.GridJs](../../../aspose.cells.gridjs/)
* assembly [Aspose.Cells.GridJs](../../../)

---

## DetailStreamJson(Stream, Workbook, string) {#detailstreamjson}

Write the JSON string for the Workbook to the stream

```csharp
public void DetailStreamJson(Stream stream, Workbook wb, string filename)
```

| Parameter | Type | Description |
| --- | --- | --- |
| stream | Stream | The stream that will be written |
| wb | Workbook | The Workbook instance |
| filename | String | The file name |

### See Also

* interface [IGridJsService](../)
* namespace [Aspose.Cells.GridJs](../../../aspose.cells.gridjs/)
* assembly [Aspose.Cells.GridJs](../../../)


