---
title: IGridJsService.Load
second_title: Aspose.Cells for .NET API Reference
description: IGridJsService method. Gets the JSON string of the file from the cache using the specified unique idset the output filename in the JSON
type: docs
url: /net/aspose.cells.gridjs/igridjsservice/load/
---
## IGridJsService.Load method

Gets the JSON string of the file from the cache using the specified unique id,set the output filename in the JSON.

```csharp
public string Load(string uid, string filename)
```

| Parameter | Type | Description |
| --- | --- | --- |
| uid | String | The unique id for the file cache. |
| filename | String | Specifies the file name in the JSON. If set to null,the default filename is: book1. |

### Return Value

The JSON string

### See Also

* interface [IGridJsService](../)
* namespace [Aspose.Cells.GridJs](../../../aspose.cells.gridjs/)
* assembly [Aspose.Cells.GridJs](../../../)


