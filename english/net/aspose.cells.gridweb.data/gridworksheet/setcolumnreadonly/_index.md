---
title: GridWorksheet.SetColumnReadonly
second_title: Aspose.Cells for .NET API Reference
description: GridWorksheet method. Sets a column to readonly so user cant delete it from client side. this is extended method of GridWeb specificallyit will not keep and take affect in actual excel file
type: docs
url: /net/aspose.cells.gridweb.data/gridworksheet/setcolumnreadonly/
---
## GridWorksheet.SetColumnReadonly method

Sets a column to readonly so user can't delete it from client side. this is extended method of GridWeb specifically,it will not keep and take affect in actual excel file

```csharp
public void SetColumnReadonly(int columnIndex, bool isReadonly)
```

| Parameter | Type | Description |
| --- | --- | --- |
| columnIndex | Int32 | column index. |
| isReadonly | Boolean | true or false. |

### See Also

* class [GridWorksheet](../)
* namespace [Aspose.Cells.GridWeb.Data](../../../aspose.cells.gridweb.data/)
* assembly [Aspose.Cells.GridWeb](../../../)


