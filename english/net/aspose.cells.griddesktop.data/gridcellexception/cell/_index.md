---
title: GridCellException.Cell
second_title: Aspose.Cells for .NET API Reference
description: GridCellException property. Gets the error cell. If the Cell is null the error is cause by an CommitNewBindRow operation that means the DataTable rejected the new created row
type: docs
url: /net/aspose.cells.griddesktop.data/gridcellexception/cell/
---
## GridCellException.Cell property

Gets the error cell. If the Cell is null, the error is cause by an CommitNewBindRow operation, that means the DataTable rejected the new created row.

```csharp
public GridCell Cell { get; }
```

### See Also

* class [GridCell](../../gridcell/)
* class [GridCellException](../)
* namespace [Aspose.Cells.GridDesktop.Data](../../../aspose.cells.griddesktop.data/)
* assembly [Aspose.Cells.GridDesktop](../../../)


