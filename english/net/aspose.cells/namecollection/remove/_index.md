...
```csharp
// Called: workBook.Worksheets.Names.Remove(rangeName);
private static void Method_String_(Workbook workBook, string worksheetName, DataTable dataTable, int startingRow, int startingCol, bool isIncludeHeaders)
        {
                    ...
                    for (int i = 0; i &lt; pivotTableColl.Count; i++)
                    {
                        for (int j = 0; j &lt; pivotTableColl[i].DataSource.Length; j++)
                        {
                            if (pivotTableColl[i].DataSource[j].IndexOf(rangeName, StringComparison.OrdinalIgnoreCase) != -1)
                            {
                                ...
                                //
                                pivotTableColl[i].RefreshData();       //System.NullReferenceException happens here
