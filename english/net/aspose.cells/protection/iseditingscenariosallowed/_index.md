---
title: IsEditingScenariosAllowed
second_title: Aspose.Cells for .NET API Reference
description: Represents if the user is allowed to edit scenarios on a protected worksheet.
type: docs
weight: 210
url: /net/aspose.cells/protection/iseditingscenariosallowed/
---
## Protection.IsEditingScenariosAllowed property

Represents if the user is allowed to edit scenarios on a protected worksheet.

```csharp
[Obsolete("Use Protection.AllowEditingScenario property instead.")]
[EditorBrowsable(EditorBrowsableState.Never)]
public bool IsEditingScenariosAllowed { get; set; }
```

### Remarks

NOTE: This member is now obsolete. Instead, please use Protection.AllowEditingScenario property. This property will be removed 12 months later since June 2010. Aspose apologizes for any inconvenience you may have experienced.

### See Also

* class [Protection](../../protection)
* namespace [Aspose.Cells](../../protection)
* assembly [Aspose.Cells](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Cells.dll -->
