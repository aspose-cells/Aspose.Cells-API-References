---
title: "PivotItem"
linktitle: "PivotItem"
second_title: "Aspose.Cells for Node.js via Java API Reference"
description: "Represents a item in a PivotField report."
type: docs
weight: 2840
url: /nodejs/aspose.cells/pivotitem/
---

## PivotItem class

Represents a item in a PivotField report.

## Methods

| Name | Description |
| --- | --- |
| [getDateTimeValue()](./getdatetimevalue/) | Gets the date time value of the pivot item If the value is null ,it will return DateTime.MinValue |
| [getDoubleValue()](./getdoublevalue/) | Gets the double value of the pivot item If the value is null or not number ,it will return 0 |
| [getFormula()](./getformula/) |  |
| [getIndex()](./getindex/) | Gets the index of the pivot item in cache field. |
| [getName()](./getname/) | Gets the name of the pivot item. |
| [getPosition()](./getposition/) | Specifying the position index in all the PivotItems,not the PivotItems under the same parent node. |
| [getPositionInSameParentNode()](./getpositioninsameparentnode/) | Specifying the position index in the PivotItems under the same parent node. |
| [getStringValue()](./getstringvalue/) | Gets the string value of the pivot item If the value is null, it will return "" |
| [getValue()](./getvalue/) | Gets the value of the pivot item |
| [hide()](./hide/) | Sets whether the pivot item is hidden. NOTE: This member is now obsolete. Instead, please use Aspose.Cells.Pivot.PivotFi |
| [isCalculatedItem()](./iscalculateditem/) |  |
| [isDetailHidden()](./isdetailhidden/) |  |
| [isFormula()](./isformula/) | Indicates whether this pivot item is a calculated formula item. |
| [isHidden()](./ishidden/) | Gets and Sets whether the pivot item is hidden. |
| [isHideDetail()](./ishidedetail/) | Gets and Sets whether the pivot item hides detail. |
| [isMissing()](./ismissing/) | Indicates whether the item is removed from the data source. True means this value has benn removed from the data source. |
| [move(count, isSameParent)](./move/) | Moves the item up or down |
| [setDetailHidden()](./setdetailhidden/) |  |
| [setFormula()](./setformula/) | Indicates whether this pivot item is a calculated formula item. |
| [setHidden()](./sethidden/) | Gets and Sets whether the pivot item is hidden. |
| [setHideDetail()](./sethidedetail/) | Gets and Sets whether the pivot item hides detail. |
| [setIndex()](./setindex/) | Gets the index of the pivot item in cache field. |
| [setName()](./setname/) | Gets the name of the pivot item. |
| [setPosition()](./setposition/) | Specifying the position index in all the PivotItems,not the PivotItems under the same parent node. |
| [setPositionInSameParentNode()](./setpositioninsameparentnode/) | Specifying the position index in the PivotItems under the same parent node. |
