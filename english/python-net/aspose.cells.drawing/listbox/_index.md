...
|item_count|Gets the number of items in the list box.|
|selected_index|Gets or sets the index number of the currently selected item in a list box or combo box.<br/>            Zero-based.|
|selected_cells|Gets the selected cells.<br/>            Returns null if the input range is not set or no item is selected|
|shadow|Indicates whether the combobox has 3-D shading.|
|selection_type|Gets or sets the selection mode of the specified list box.|
|page_change|Specifies the amount by which the control's value is changed <br/>            when the user clicks on the scrollbar's page up or page down region.|
## Methods
| Name | Description |
| :- | :- |
...
|selected_item(item_index, is_selected)|Sets whether the item is selected|
|is_selected(item_index)|Indicates whether the item is selected.|
