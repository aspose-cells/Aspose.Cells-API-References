---
title: "FileFontSource Class"
linktitle: "FileFontSource"
articleTitle: "FileFontSource"
second_title: "Aspose.Cells for PHP via Java"
description: "Represents the single TrueType font file stored in the file system."
type: docs
weight: 2230
url: /php/aspose.cells/filefontsource/
---

## FileFontSource class

Represents the single TrueType font file stored in the file system.

## Constructors

| Name | Description |
| --- | --- |
| [FileFontSource](#constructor) | Ctor. |

## Properties

| Name | Type | Description |
| --- | --- | --- |
| [FilePath](#filepath) | String | Path to font file. |
| [Type](#type) | Number | Returns the type of the font source. The value of the property is FontSourceType integer constant. |

### FileFontSource(filePath) {#constructor}

Ctor.

| Parameter | Type | Description |
| --- | --- | --- |
| filePath | String | path to font file |

### FileFontSource.FilePath property {#filepath}

Path to font file.

**Type:** String

### FileFontSource.Type property {#type}

Returns the type of the font source. The value of the property is FontSourceType integer constant.

**Type:** Number
