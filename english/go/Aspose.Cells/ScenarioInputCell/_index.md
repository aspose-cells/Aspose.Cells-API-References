---
title: ScenarioInputCell Class 
linktitle: ScenarioInputCell
second_title: Aspose.Cells for Go API Reference
description: 'ScenarioInputCell class. Encapsulates the object that represents scenarioinputcell in Go.'
type: docs
weight: 200
url: /go/aspose.cells/scenarioinputcell/
---

## ScenarioInputCell class

Represents input cell for the scenario.

```go

type ScenarioInputCell struct 

scenarioinputcell, _ := asposecells.NewScenarioInputCell()

```
## Constructors

| Method | Description |
| --- | --- |
|[NewScenarioInputCell](./newscenarioinputcell/) | Constructs from an implementation object. | 

## Methods

| Method | Description |
| --- | --- |
|[IsNull](./isnull/) | Checks whether the implementation object is nullptr. | 
|[GetRow](./getrow/) | Gets and sets the row index of the input cell. | 
|[GetColumn](./getcolumn/) | Gets and sets the column index of the input cell. | 
|[GetName](./getname/) | Gets and sets the input cell address. | 
|[GetValue](./getvalue/) | Gets and sets value of the input cell. | 
|[SetValue](./setvalue/) | Gets and sets value of the input cell. | 
|[IsDeleted](./isdeleted/) | Indicates whether input cell is deleted. | 
|[SetIsDeleted](./setisdeleted/) | Indicates whether input cell is deleted. | 
