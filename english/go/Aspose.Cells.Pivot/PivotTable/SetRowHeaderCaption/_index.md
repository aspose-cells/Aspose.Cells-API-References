---
title: SetRowHeaderCaption Method 
linktitle: SetRowHeaderCaption
second_title: Aspose.Cells for Go API Reference
description: 'SetRowHeaderCaption method. Encapsulates the function that represents setrowheadercaption in Go.'
type: docs
weight: 200
url: /go/aspose.cells.pivot/pivottable/setrowheadercaption/
---

## SetRowHeaderCaption function

Gets the Row Header Caption of the PivotTable.

```go

func (instance *PivotTable) SetRowHeaderCaption_String(value string)  error

```

## Remarks


## See Also

* Class [PivotTable](../)
* Namespace [Aspose.Cells.Pivot](../../)
* Library [Aspose.Cells for Go](../../../)
