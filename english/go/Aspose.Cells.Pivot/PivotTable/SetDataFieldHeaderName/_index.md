---
title: SetDataFieldHeaderName Method 
linktitle: SetDataFieldHeaderName
second_title: Aspose.Cells for Go API Reference
description: 'SetDataFieldHeaderName method. Encapsulates the function that represents setdatafieldheadername in Go.'
type: docs
weight: 200
url: /go/aspose.cells.pivot/pivottable/setdatafieldheadername/
---

## SetDataFieldHeaderName function

Gets and sets the name of the value area field header in the PivotTable.

```go

func (instance *PivotTable) SetDataFieldHeaderName_String(value string)  error

```

## Remarks


## See Also

* Class [PivotTable](../)
* Namespace [Aspose.Cells.Pivot](../../)
* Library [Aspose.Cells for Go](../../../)
