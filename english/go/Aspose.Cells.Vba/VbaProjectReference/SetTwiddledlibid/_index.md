---
title: SetTwiddledlibid Method 
linktitle: SetTwiddledlibid
second_title: Aspose.Cells for Go API Reference
description: 'SetTwiddledlibid method. Encapsulates the function that represents settwiddledlibid in Go.'
type: docs
weight: 200
url: /go/aspose.cells.vba/vbaprojectreference/settwiddledlibid/
---

## SetTwiddledlibid function

Gets and sets the twiddled Libid of the reference.

```go

func (instance *VbaProjectReference) SetTwiddledlibid_String(value string)  error

```

## Remarks


## See Also

* Class [VbaProjectReference](../)
* Namespace [Aspose.Cells.Vba](../../)
* Library [Aspose.Cells for Go](../../../)
