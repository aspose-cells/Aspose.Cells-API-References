---
title: LineType Aufzählung
second_title: Aspose.Cells for Python via .NET API Referenzen
description:
type: docs
weight: 950
url: /de/python-net/aspose.cells.drawing/linetype/
is_root: false
---
##  LineType Aufzählung
Zählt den Typ der [Picture](/cells/de/python-net/aspose.cells.drawing/picture)-Grenze oder [Chart](/cells/de/python-net/aspose.cells.charts/chart)-Linie auf.



Der Typ LineType macht die folgenden Member verfügbar:

###  Felder
| Feld| Beschreibung|
| :- | :- |
| DARK_GRAY | Stellt eine dunkelgraue Linie dar.|
| DASH | Stellen Sie eine gestrichelte Linie dar.|
| DASH_DOT | Stellt eine Strichpunktlinie dar|
| DASH_DOT_DOT | Stellt eine Strich-Punkt-Punkt-Linie dar.|
| DOT | Stellt eine gepunktete Linie dar.|
| LIGHT_GRAY | Stellt eine hellgraue Linie dar.|
| MEDIUM_GRAY | Stellt eine mittelgraue Linie dar.|
| SOLID | Stellen Sie eine durchgezogene Linie dar.|



###  Siehe auch
* Modul [aspose.cells.drawing](..)
* Klasse [Chart](/cells/de/python-net/aspose.cells.charts/chart)
* Klasse [Picture](/cells/de/python-net/aspose.cells.drawing/picture)
