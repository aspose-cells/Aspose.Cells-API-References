---
title: copy_comments_in_range Methode
second_title: Aspose.Cells for Python via .NET API Referenzen
description:
type: docs
weight: 370
url: /de/python-net/aspose.cells.drawing/shapecollection/copy_comments_in_range/
is_root: false
---
##  copy_comments_in_range(shapes, ca, dest_row, dest_column) {#ShapeCollection-CellArea-int-int}
Kopieren Sie alle Kommentare im Bereich.



```python
def copy_comments_in_range(self, shapes, ca, dest_row, dest_column):
    ...
```


| Parameter| Typ| Beschreibung|
| :- | :- | :- |
| shapes | [ShapeCollection](/cells/de/python-net/aspose.cells.drawing/shapecollection) | Die Quellformen.|
| ca | [CellArea](/cells/de/python-net/aspose.cells/cellarea) | Der Quellbereich.|
| dest_row | int | Die Startreihe des Zielbereichs.|
| dest_column | int | Die Startspalte des Zielbereichs.|



###  Siehe auch
* Modul [aspose.cells.drawing](../../)
* Klasse [ShapeCollection](/cells/de/python-net/aspose.cells.drawing/shapecollection)
