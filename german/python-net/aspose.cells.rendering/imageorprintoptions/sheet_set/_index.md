---
title: sheet_set Eigentum
second_title: Aspose.Cells for Python via .NET API Referenzen
description:
type: docs
weight: 290
url: /de/python-net/aspose.cells.rendering/imageorprintoptions/sheet_set/
is_root: false
---
##  sheet_set Eigentum

Ruft die zu rendernden Blätter ab oder legt diese fest. Standard sind alle sichtbaren Blätter in der Arbeitsmappe: [SheetSet.visible](/cells/de/python-net/aspose.cells.rendering/sheetset#visible).

###  Bemerkungen

Der Satz wird ignoriert, wenn er in [SheetRender](/cells/de/python-net/aspose.cells.rendering/sheetrender) verwendet wird
###  Definition:
```python
@property
def sheet_set(self):
    ...
@sheet_set.setter
def sheet_set(self, value):
    ...
```

###  Siehe auch
* Modul [aspose.cells.rendering](../../)
* Klasse [ImageOrPrintOptions](/cells/de/python-net/aspose.cells.rendering/imageorprintoptions)
* Klasse [SheetRender](/cells/de/python-net/aspose.cells.rendering/sheetrender)
* Klasse [SheetSet](/cells/de/python-net/aspose.cells.rendering/sheetset)
