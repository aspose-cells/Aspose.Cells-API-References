---
title: get_even_header Methode
second_title: Aspose.Cells for Python via .NET API Referenzen
description:
type: docs
weight: 70
url: /de/python-net/aspose.cells/pagesetup/get_even_header/
is_root: false
---
##  get_even_header(section) {#int}
Ruft ein Skript ab, das den geraden Header einer Excel-Datei formatiert.



```python
def get_even_header(self, section):
    ...
```


| Parameter| Typ| Beschreibung|
| :- | :- | :- |
| section | int | 0: Linker Abschnitt, 1: Mittlerer Abschnitt, 2: Rechter Abschnitt.|



###  Siehe auch
* Modul [aspose.cells](../../)
* Klasse [PageSetup](/cells/de/python-net/aspose.cells/pagesetup)
