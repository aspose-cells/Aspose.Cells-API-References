---
title: AutoFitMergedCellsType Aufzählung
second_title: Aspose.Cells for Python via .NET API Referenzen
description:
type: docs
weight: 1760
url: /de/python-net/aspose.cells/autofitmergedcellstype/
is_root: false
---
##  AutoFitMergedCellsType Aufzählung
Stellt den Typ der automatisch angepassten verbundenen Zellen dar.



Der Typ AutoFitMergedCellsType macht die folgenden Member verfügbar:

###  Felder
| Feld| Beschreibung|
| :- | :- |
| NONE | Ignorieren Sie verbundene Zellen.|
| FIRST_LINE | Erweitert nur die Höhe der ersten Zeile.|
| LAST_LINE |Erweitert nur die Höhe der letzten Zeile.|
| EACH_LINE | Erweitert nur die Höhe jeder Zeile.|



###  Siehe auch
* Modul [aspose.cells](..)
