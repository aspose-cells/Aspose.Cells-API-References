---
title: طريقة add
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 20
url: /ar/python-net/aspose.cells/horizontalpagebreakcollection/add/
is_root: false
---
##  add(row) {#int}
يضيف فاصل صفحات أفقيًا إلى المجموعة.


###  عائدات

[HorizontalPageBreak](/cells/ar/python-net/aspose.cells/horizontalpagebreak) فهرس العنصر.


```python
def add(self, row):
    ...
```


| معامل| يكتب| وصف|
| :- | :- | :- |
| row | int | Cell صف فهرس ، على أساس الصفر.|
###  ملاحظات

تتم إضافة فاصل الصفحة في الجزء العلوي الأيسر من الخلية.
يرجى تعيين فاصل صفحة أفقي وفاصل صفحة عمودي في نفس الوقت.

##  add(cell_name) {#str}
يضيف فاصل صفحات أفقيًا إلى المجموعة.


###  عائدات

[HorizontalPageBreak](/cells/ar/python-net/aspose.cells/horizontalpagebreak) فهرس العنصر.


```python
def add(self, cell_name):
    ...
```


| معامل| يكتب| وصف|
| :- | :- | :- |
| cell_name | str | Cell الاسم.|
###  ملاحظات

تتم إضافة فاصل الصفحة في الجزء العلوي الأيسر من الخلية.
يرجى تعيين فاصل صفحة أفقي وفاصل صفحة عمودي في نفس الوقت.

##  add(row, column) {#int-int}
يضيف فاصل صفحات أفقيًا إلى المجموعة.


###  عائدات

[HorizontalPageBreak](/cells/ar/python-net/aspose.cells/horizontalpagebreak) فهرس العنصر.


```python
def add(self, row, column):
    ...
```


| معامل| يكتب| وصف|
| :- | :- | :- |
| row | int | Cell صف فهرس ، على أساس الصفر.|
| column | int | Cell عمود فهرس ، قائم على الصفر.|
###  ملاحظات

تتم إضافة فاصل الصفحة في الجزء العلوي الأيسر من الخلية.
يرجى تعيين فاصل صفحة أفقي وفاصل صفحة عمودي في نفس الوقت.

##  add(row, start_column, end_column) {#int-int-int}
يضيف فاصل صفحات أفقيًا إلى المجموعة.


###  عائدات

[HorizontalPageBreak](/cells/ar/python-net/aspose.cells/horizontalpagebreak) فهرس العنصر.


```python
def add(self, row, start_column, end_column):
    ...
```


| معامل| يكتب| وصف|
| :- | :- | :- |
| row | int | مؤشر الصف ، على أساس الصفر.|
| start_column | int | مؤشر عمود البداية ، قائم على الصفر.|
| end_column | int | مؤشر عمود النهاية ، قائم على الصفر.|
###  ملاحظات

تستخدم هذه الطريقة في add فاصل صفحات أفقي داخل منطقة الطباعة.


###  أنظر أيضا

* وحدة [aspose.cells](../../)
* فئة [HorizontalPageBreak](/cells/ar/python-net/aspose.cells/horizontalpagebreak)
* فئة [HorizontalPageBreakCollection](/cells/ar/python-net/aspose.cells/horizontalpagebreakcollection)
