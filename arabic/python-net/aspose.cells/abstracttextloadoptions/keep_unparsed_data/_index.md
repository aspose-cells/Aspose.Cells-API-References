---
title: keep_unparsed_data الملكية
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 160
url: /ar/python-net/aspose.cells/abstracttextloadoptions/keep_unparsed_data/
is_root: false
---
##  keep_unparsed_data الملكية

سواء احتفظ بالبيانات التي لم يتم تحليلها في الذاكرة للمصنف عند تحميله من ملف القالب أم لا. الافتراضي هو true.

###  ملاحظات

بالنسبة للسيناريوهات التي يحتاجها المستخدم فقط لقراءة بعض المحتويات من ملف القالب ولا يحتاج إلى حفظ المصنف مرة أخرى ،
قد يؤدي تعيين هذه الخاصية على أنها خطأ إلى تحسين الأداء ، خاصة عند استخدامها مع نوع من LoadFilter ،
###  تعريف:
```python
@property
def keep_unparsed_data(self):
    ...
@keep_unparsed_data.setter
def keep_unparsed_data(self, value):
    ...
```

###  أنظر أيضا
* وحدة [aspose.cells](../../)
* فئة [AbstractTextLoadOptions](/cells/ar/python-net/aspose.cells/abstracttextloadoptions)
