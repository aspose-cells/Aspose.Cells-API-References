---
title: merged_cells الملكية
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 1150
url: /ar/python-net/aspose.cells/cells/merged_cells/
is_root: false
---
##  merged_cells الملكية

يحصل على مجموعة الخلايا المدمجة.

###  ملاحظات

في هذه المجموعة ، كل عنصر عبارة عن هيكل [CellArea](/cells/ar/python-net/aspose.cells/cellarea) يمثل مساحة من الخلايا المدمجة.
###  تعريف:
```python
@property
def merged_cells(self):
    ...
```

###  أنظر أيضا
* وحدة [aspose.cells](../../)
* فئة [CellArea](/cells/ar/python-net/aspose.cells/cellarea)
* فئة [Cells](/cells/ar/python-net/aspose.cells/cells)
