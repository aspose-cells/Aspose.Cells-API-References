---
title: طريقة advanced_filter
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 30
url: /ar/python-net/aspose.cells/worksheet/advanced_filter/
is_root: false
---
##  advanced_filter(is_filter, list_range, criteria_range, copy_to, unique_record_only) {#bool-str-str-str-bool}
يقوم بتصفية البيانات باستخدام معايير معقدة.



```python
def advanced_filter(self, is_filter, list_range, criteria_range, copy_to, unique_record_only):
    ...
```


| معامل| يكتب| وصف|
| :- | :- | :- |
| is_filter | bool | يشير إلى ما إذا كانت تصفية القائمة في مكانها.|
| list_range | str | نطاق القائمة.|
| criteria_range | str | نطاق المعايير.|
| copy_to | str | النطاق الذي يتم فيه نسخ البيانات.|
| unique_record_only | bool | عرض أو نسخ الصفوف الفريدة فقط.|



###  أنظر أيضا
* وحدة [aspose.cells](../../)
* فئة [Worksheet](/cells/ar/python-net/aspose.cells/worksheet)
