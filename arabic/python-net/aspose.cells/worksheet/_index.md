---
title: Worksheet الدرجة
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 1620
url: /ar/python-net/aspose.cells/worksheet/
is_root: false
---
##  Worksheet الدرجة
لتغليف الكائن الذي يمثل ورقة عمل واحدة.



يكشف نوع Worksheet الأعضاء التالية:

###  ملكيات
| ملكية| وصف|
| :- | :- |
| [protection](/cells/ar/python-net/aspose.cells/worksheet/protection) | يمثل الأنواع المختلفة من خيارات الحماية المتاحة لورقة العمل.|
| [unique_id](/cells/ar/python-net/aspose.cells/worksheet/unique_id) | الحصول على المعرف الفريد وتعيينه ، فهو نفس {15DB5C3C-A5A1-48AF-8F25-3D86AC232D4F}.|
| [workbook](/cells/ar/python-net/aspose.cells/worksheet/workbook) | يحصل على كائن المصنف الذي يحتوي على هذه الورقة.|
| [cells](/cells/ar/python-net/aspose.cells/worksheet/cells) | الحصول على مجموعة [Worksheet.cells](/cells/ar/python-net/aspose.cells/worksheet#cells).|
| [query_tables](/cells/ar/python-net/aspose.cells/worksheet/query_tables) | يحصل على [QueryTableCollection](/cells/ar/python-net/aspose.cells/querytablecollection) في ورقة العمل.|
| [pivot_tables](/cells/ar/python-net/aspose.cells/worksheet/pivot_tables) | يحصل على جميع الجداول المحورية في ورقة العمل هذه.|
| [type](/cells/ar/python-net/aspose.cells/worksheet/type) | يمثل نوع ورقة العمل.|
| [name](/cells/ar/python-net/aspose.cells/worksheet/name) | الحصول على اسم ورقة العمل أو تعيينه.|
| [show_formulas](/cells/ar/python-net/aspose.cells/worksheet/show_formulas) | يشير إلى ما إذا كان سيتم إظهار الصيغ أو نتائجها.|
| [is_gridlines_visible](/cells/ar/python-net/aspose.cells/worksheet/is_gridlines_visible) | الحصول على أو تعيين قيمة تشير إلى ما إذا كانت خطوط الشبكة مرئية أم لا. الافتراضي هو الصحيح.|
| [is_row_column_headers_visible](/cells/ar/python-net/aspose.cells/worksheet/is_row_column_headers_visible) | الحصول على أو تعيين قيمة تشير إلى ما إذا كانت ورقة العمل ستعرض رؤوس الصفوف والأعمدة.<br/> الافتراضي هو الصحيح.|
| [pane_state](/cells/ar/python-net/aspose.cells/worksheet/pane_state) |يشير إلى ما إذا كان الجزء يحتوي على انقسامات أفقية أو رأسية ، وما إذا كانت هذه الانقسامات مجمدة.|
| [display_zeros](/cells/ar/python-net/aspose.cells/worksheet/display_zeros) | صواب إذا تم عرض القيم الصفرية.|
| [display_right_to_left](/cells/ar/python-net/aspose.cells/worksheet/display_right_to_left) | يشير إلى ما إذا كانت ورقة العمل المحددة معروضة من اليمين إلى اليسار بدلاً من عرضها من اليسار إلى اليمين.<br/> الافتراضي هو خطأ.|
| [is_outline_shown](/cells/ar/python-net/aspose.cells/worksheet/is_outline_shown) | يشير إلى ما إذا كان سيتم إظهار مخطط تفصيلي.|
| [is_selected](/cells/ar/python-net/aspose.cells/worksheet/is_selected) | يشير إلى ما إذا كانت ورقة العمل هذه محددة عند فتح المصنف.|
| [list_objects](/cells/ar/python-net/aspose.cells/worksheet/list_objects) | يحصل على كافة ListObjects في ورقة العمل هذه.|
| [tab_id](/cells/ar/python-net/aspose.cells/worksheet/tab_id) | يحدد المعرف الداخلي للورقة.|
| [horizontal_page_breaks](/cells/ar/python-net/aspose.cells/worksheet/horizontal_page_breaks) | الحصول على مجموعة [HorizontalPageBreakCollection](/cells/ar/python-net/aspose.cells/horizontalpagebreakcollection).|
| [vertical_page_breaks](/cells/ar/python-net/aspose.cells/worksheet/vertical_page_breaks) | الحصول على مجموعة [VerticalPageBreakCollection](/cells/ar/python-net/aspose.cells/verticalpagebreakcollection).|
| [hyperlinks](/cells/ar/python-net/aspose.cells/worksheet/hyperlinks) | الحصول على مجموعة [HyperlinkCollection](/cells/ar/python-net/aspose.cells/hyperlinkcollection).|
| [page_setup](/cells/ar/python-net/aspose.cells/worksheet/page_setup) | يمثل وصف إعداد الصفحة في هذه الورقة.|
| [auto_filter](/cells/ar/python-net/aspose.cells/worksheet/auto_filter) | يمثل عامل التصفية التلقائي لورقة العمل المحددة.|
| [has_autofilter](/cells/ar/python-net/aspose.cells/worksheet/has_autofilter) | يشير إلى ما إذا كانت ورقة العمل هذه تحتوي على عامل تصفية تلقائي.|
| [transition_evaluation](/cells/ar/python-net/aspose.cells/worksheet/transition_evaluation) | يشير إلى ما إذا كان خيار تقييم الصيغة الانتقالية (توافق Lotus) ممكّنًا.|
| [transition_entry](/cells/ar/python-net/aspose.cells/worksheet/transition_entry) | يشير إلى ما إذا كان الخيار Transition Formula Entry (توافق Lotus) ممكّنًا.|
| [visibility_type](/cells/ar/python-net/aspose.cells/worksheet/visibility_type) | يشير إلى الحالة المرئية لهذه الورقة.|
| [is_visible](/cells/ar/python-net/aspose.cells/worksheet/is_visible) | يمثل إذا كانت ورقة العمل مرئية.|
| [sparkline_group_collection](/cells/ar/python-net/aspose.cells/worksheet/sparkline_group_collection) | الحصول على مجموعة مجموعة خط المؤشر في ورقة العمل.|
| [sparkline_groups](/cells/ar/python-net/aspose.cells/worksheet/sparkline_groups) |يحصل على مجموعات خط المؤشر في ورقة العمل.|
| [charts](/cells/ar/python-net/aspose.cells/worksheet/charts) | يحصل على مجموعة [Chart](/cells/ar/python-net/aspose.cells.charts/chart)|
| [comments](/cells/ar/python-net/aspose.cells/worksheet/comments) | الحصول على مجموعة [Comment](/cells/ar/python-net/aspose.cells/comment).|
| [pictures](/cells/ar/python-net/aspose.cells/worksheet/pictures) | يحصل على مجموعة [Picture](/cells/ar/python-net/aspose.cells.drawing/picture).|
| [text_boxes](/cells/ar/python-net/aspose.cells/worksheet/text_boxes) | يحصل على مجموعة [TextBox](/cells/ar/python-net/aspose.cells.drawing/textbox).|
| [check_boxes](/cells/ar/python-net/aspose.cells/worksheet/check_boxes) | يحصل على مجموعة [CheckBox](/cells/ar/python-net/aspose.cells.drawing/checkbox).|
| [ole_objects](/cells/ar/python-net/aspose.cells/worksheet/ole_objects) | يمثل مجموعة من [OleObject](/cells/ar/python-net/aspose.cells.drawing/oleobject) في ورقة عمل.|
| [shapes](/cells/ar/python-net/aspose.cells/worksheet/shapes) | إرجاع كافة أشكال الرسم في ورقة العمل هذه.|
| [slicers](/cells/ar/python-net/aspose.cells/worksheet/slicers) | احصل على مجموعة القطاعة في ورقة العمل|
| [timelines](/cells/ar/python-net/aspose.cells/worksheet/timelines) | احصل على مجموعة الجدول الزمني في ورقة العمل|
| [index](/cells/ar/python-net/aspose.cells/worksheet/index) | يحصل على فهرس الأوراق في مجموعة أوراق العمل.|
| [is_protected](/cells/ar/python-net/aspose.cells/worksheet/is_protected) | يشير إلى ما إذا كانت ورقة العمل محمية.|
| [validations](/cells/ar/python-net/aspose.cells/worksheet/validations) | يحصل على مجموعة إعدادات التحقق من صحة البيانات في ورقة العمل.|
| [allow_edit_ranges](/cells/ar/python-net/aspose.cells/worksheet/allow_edit_ranges) | يحصل على مجموعة نطاق التحرير المسموح بها في ورقة العمل.|
| [error_check_options](/cells/ar/python-net/aspose.cells/worksheet/error_check_options) | يحصل على إعداد التحقق من الخطأ المطبق على نطاقات معينة.|
| [outline](/cells/ar/python-net/aspose.cells/worksheet/outline) | يحصل على المخطط التفصيلي في ورقة العمل هذه.|
| [first_visible_row](/cells/ar/python-net/aspose.cells/worksheet/first_visible_row) | يمثل أول فهرس للصف المرئي.|
| [first_visible_column](/cells/ar/python-net/aspose.cells/worksheet/first_visible_column) | يمثل أول مؤشر عمود مرئي.|
| [zoom](/cells/ar/python-net/aspose.cells/worksheet/zoom) | يمثل عامل القياس بالنسبة المئوية ويجب أن يكون بين 10 و 400.|
| [view_type](/cells/ar/python-net/aspose.cells/worksheet/view_type) | يحصل ويضبط نوع العرض.|
| [is_page_break_preview](/cells/ar/python-net/aspose.cells/worksheet/is_page_break_preview) | يشير إلى ما إذا كانت ورقة العمل المحددة ستظهر في العرض العادي أو معاينة فاصل الصفحة.|
| [is_ruler_visible](/cells/ar/python-net/aspose.cells/worksheet/is_ruler_visible) |يشير إلى ما إذا كانت المسطرة مرئية أم لا.|
| [tab_color](/cells/ar/python-net/aspose.cells/worksheet/tab_color) | يمثل لون علامة تبويب ورقة العمل.|
| [code_name](/cells/ar/python-net/aspose.cells/worksheet/code_name) | يحصل على الاسم الرمزي لورقة العمل.|
| [background_image](/cells/ar/python-net/aspose.cells/worksheet/background_image) | الحصول على صورة خلفية ورقة العمل وتعيينها.|
| [conditional_formattings](/cells/ar/python-net/aspose.cells/worksheet/conditional_formattings) | الحصول على التنسيق الشرطي في ورقة العمل.|
| [active_cell](/cells/ar/python-net/aspose.cells/worksheet/active_cell) | الحصول على الخلية النشطة أو تعيينها في ورقة العمل.|
| [custom_properties](/cells/ar/python-net/aspose.cells/worksheet/custom_properties) | يحصل على كائن يمثل<br/> معلومات المعرف المرتبطة بورقة العمل.|
| [smart_tag_setting](/cells/ar/python-net/aspose.cells/worksheet/smart_tag_setting) | يحصل على كل [SmartTagCollection](/cells/ar/python-net/aspose.cells.markup/smarttagcollection) كائنات من ورقة العمل.|
| [scenarios](/cells/ar/python-net/aspose.cells/worksheet/scenarios) | يحصل على مجموعة [Scenario](/cells/ar/python-net/aspose.cells/scenario).|
| [cell_watches](/cells/ar/python-net/aspose.cells/worksheet/cell_watches) | الحصول على مجموعة من الخلايا في ورقة العمل هذه يتم مشاهدتها في "نافذة المراقبة".|


###  طُرق
| طريقة| وصف|
| :- | :- |
| [freeze_panes(row, column, freezed_rows, freezed_columns)](/cells/ar/python-net/aspose.cells/worksheet/freeze_panes/#int-int-int-int) | تجميد الأجزاء في الخلية المحددة في ورقة العمل.|
| [freeze_panes(cell_name, freezed_rows, freezed_columns)](/cells/ar/python-net/aspose.cells/worksheet/freeze_panes/#str-int-int) | تجميد الأجزاء في الخلية المحددة في ورقة العمل.|
| [copy(source_sheet)](/cells/ar/python-net/aspose.cells/worksheet/copy/#Worksheet) | ينسخ المحتويات والتنسيقات من ورقة عمل أخرى.|
| [copy(source_sheet, copy_options)](/cells/ar/python-net/aspose.cells/worksheet/copy/#Worksheet-CopyOptions) | ينسخ المحتويات والتنسيقات من ورقة عمل أخرى.|
| [auto_fit_column(column_index, first_row, last_row)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_column/#int-int-int) | يتم ضبط عرض العمود تلقائيًا.|
| [auto_fit_column(column_index)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_column/#int) | يتم ضبط عرض العمود تلقائيًا.|
| [auto_fit_columns()](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_columns/#) | احتواء تلقائي لكافة الأعمدة في ورقة العمل هذه.|
| [auto_fit_columns(options)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_columns/#AutoFitterOptions) | احتواء تلقائي لكافة الأعمدة في ورقة العمل هذه.|
| [auto_fit_columns(first_column, last_column)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_columns/#int-int) | يتم ضبط عرض العمود تلقائيًا.|
| [auto_fit_columns(first_column, last_column, options)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_columns/#int-int-AutoFitterOptions) | يتم ضبط عرض العمود تلقائيًا.|
| [auto_fit_columns(first_row, first_column, last_row, last_column)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_columns/#int-int-int-int) | يتم ضبط عرض العمود تلقائيًا.|
| [auto_fit_columns(first_row, first_column, last_row, last_column, options)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_columns/#int-int-int-int-AutoFitterOptions) | يتم ضبط عرض العمود تلقائيًا.|
| [auto_fit_row(row_index, first_column, last_column)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_row/#int-int-int) | يناسب ارتفاع الصف تلقائيًا.|
| [auto_fit_row(row_index, first_column, last_column, options)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_row/#int-int-int-AutoFitterOptions) | يناسب ارتفاع الصف تلقائيًا.|
| [auto_fit_row(start_row, end_row, start_column, end_column)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_row/#int-int-int-int) | احتواء تلقائي لارتفاع الصف في نطاق مستطيل.|
| [auto_fit_row(row_index)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_row/#int) | يناسب ارتفاع الصف تلقائيًا.|
| [auto_fit_rows()](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_rows/#) | احتواء تلقائي لجميع الصفوف في ورقة العمل هذه.|
| [auto_fit_rows(only_auto)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_rows/#bool) | احتواء تلقائي لجميع الصفوف في ورقة العمل هذه.|
| [auto_fit_rows(options)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_rows/#AutoFitterOptions) | احتواء تلقائي لجميع الصفوف في ورقة العمل هذه.|
| [auto_fit_rows(start_row, end_row)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_rows/#int-int) |احتواء تلقائي لارتفاع الصف في نطاق.|
| [auto_fit_rows(start_row, end_row, options)](/cells/ar/python-net/aspose.cells/worksheet/auto_fit_rows/#int-int-AutoFitterOptions) |احتواء تلقائي لارتفاع الصف في نطاق.|
| [protect(type)](/cells/ar/python-net/aspose.cells/worksheet/protect/#ProtectionType) | يحمي ورقة العمل.|
| [protect(type, password, old_password)](/cells/ar/python-net/aspose.cells/worksheet/protect/#ProtectionType-str-str) | يحمي ورقة العمل.|
| [unprotect()](/cells/ar/python-net/aspose.cells/worksheet/unprotect/#) | عدم حماية ورقة العمل.|
| [unprotect(password)](/cells/ar/python-net/aspose.cells/worksheet/unprotect/#str) | عدم حماية ورقة العمل.|
| [calculate_formula(formula)](/cells/ar/python-net/aspose.cells/worksheet/calculate_formula/#str) | تحسب صيغة.|
| [calculate_formula(formula, opts)](/cells/ar/python-net/aspose.cells/worksheet/calculate_formula/#str-CalculationOptions) | تحسب صيغة.|
| [calculate_formula(recursive, ignore_error, custom_function)](/cells/ar/python-net/aspose.cells/worksheet/calculate_formula/#bool-bool-ICustomFunction) | تحسب جميع الصيغ في ورقة العمل هذه.|
| [calculate_formula(options, recursive)](/cells/ar/python-net/aspose.cells/worksheet/calculate_formula/#CalculationOptions-bool) | تحسب جميع الصيغ في ورقة العمل هذه.|
| [get_panes()](/cells/ar/python-net/aspose.cells/worksheet/get_panes/#) | يحصل على أجزاء النافذة.|
| [get_freezed_panes(row, column, freezed_rows, freezed_columns)](/cells/ar/python-net/aspose.cells/worksheet/get_freezed_panes/#any-any-any-any) | يحصل على ألواح التجميد.|
| [split()](/cells/ar/python-net/aspose.cells/worksheet/split/#) | نافذة انقسام.|
| [un_freeze_panes()](/cells/ar/python-net/aspose.cells/worksheet/un_freeze_panes/#) | يقوم بإلغاء تجميد الأجزاء في ورقة العمل.|
| [remove_split()](/cells/ar/python-net/aspose.cells/worksheet/remove_split/#) | يزيل انقسام النافذة.|
| [add_page_breaks(cell_name)](/cells/ar/python-net/aspose.cells/worksheet/add_page_breaks/#str) | يضيف فاصل صفحة.|
| [advanced_filter(is_filter, list_range, criteria_range, copy_to, unique_record_only)](/cells/ar/python-net/aspose.cells/worksheet/advanced_filter/#bool-str-str-str-bool) | يقوم بتصفية البيانات باستخدام معايير معقدة.|
| [remove_auto_filter()](/cells/ar/python-net/aspose.cells/worksheet/remove_auto_filter/#) | يزيل عامل التصفية التلقائي لورقة العمل.|
| [set_visible(is_visible, ignore_error)](/cells/ar/python-net/aspose.cells/worksheet/set_visible/#bool-bool) | يضبط الخيارات المرئية.|
| [select_range(start_row, start_column, total_rows, total_columns, remove_others)](/cells/ar/python-net/aspose.cells/worksheet/select_range/#int-int-int-int-bool) | يختار النطاق.|
| [remove_all_drawing_objects()](/cells/ar/python-net/aspose.cells/worksheet/remove_all_drawing_objects/#) | يزيل كافة الكائنات الرسومية في ورقة العمل هذه.|
| [clear_comments()](/cells/ar/python-net/aspose.cells/worksheet/clear_comments/#) | يمسح كل التعليقات في جدول بيانات المصمم.|
| [move_to(index)](/cells/ar/python-net/aspose.cells/worksheet/move_to/#int) | ينقل الورقة إلى موقع آخر في جدول البيانات.|
| [replace(old_string, new_string)](/cells/ar/python-net/aspose.cells/worksheet/replace/#str-str) | يستبدل نص كل الخلايا بسلسلة جديدة.|
| [get_selected_ranges()](/cells/ar/python-net/aspose.cells/worksheet/get_selected_ranges/#) | الحصول على نطاقات الخلايا المحددة في جدول بيانات المصمم.|
| [set_background(picture_data)](/cells/ar/python-net/aspose.cells/worksheet/set_background/#bytes) | يضبط صورة خلفية ورقة العمل.|
| [get_printing_page_breaks(options)](/cells/ar/python-net/aspose.cells/worksheet/get_printing_page_breaks/#aspose.cells.rendering.ImageOrPrintOptions) | يحصل على فواصل الصفحات التلقائية.|
| [start_access_cache(opts)](/cells/ar/python-net/aspose.cells/worksheet/start_access_cache/#AccessCacheOptions) | يبدأ الجلسة التي تستخدم ذاكرات التخزين المؤقت للوصول إلى البيانات الموجودة في ورقة العمل هذه.|
| [close_access_cache(opts)](/cells/ar/python-net/aspose.cells/worksheet/close_access_cache/#AccessCacheOptions) |إغلاق الجلسة التي تستخدم ذاكرات التخزين المؤقت للوصول إلى البيانات الموجودة في ورقة العمل هذه.|
| [xml_map_query(path, xml_map)](/cells/ar/python-net/aspose.cells/worksheet/xml_map_query/#str-XmlMap) | مناطق خلايا الاستعلام التي تم تعيينها / ربطها بالمسار المحدد لخريطة xml.|
| [refresh_pivot_tables()](/cells/ar/python-net/aspose.cells/worksheet/refresh_pivot_tables/#) | يحدّث كل جداول PivotTables في ورقة العمل هذه.|



###  أمثلة

```python
from aspose.cells import Workbook

workbook = Workbook()
sheet = workbook.worksheets[0]
# Freeze panes at "AS40" with 10 rows and 10 columns
sheet.freeze_panes("AS40", 10, 10)
# Add a hyperlink in Cell A1
sheet.hyperlinks.add("A1", 1, 1, "http://www.aspose.com")

```

###  أنظر أيضا
* وحدة [aspose.cells](..)
* فئة [Chart](/cells/ar/python-net/aspose.cells.charts/chart)
* فئة [CheckBox](/cells/ar/python-net/aspose.cells.drawing/checkbox)
* فئة [Comment](/cells/ar/python-net/aspose.cells/comment)
* فئة [HorizontalPageBreakCollection](/cells/ar/python-net/aspose.cells/horizontalpagebreakcollection)
* فئة [HyperlinkCollection](/cells/ar/python-net/aspose.cells/hyperlinkcollection)
* فئة [OleObject](/cells/ar/python-net/aspose.cells.drawing/oleobject)
* فئة [Picture](/cells/ar/python-net/aspose.cells.drawing/picture)
* فئة [QueryTableCollection](/cells/ar/python-net/aspose.cells/querytablecollection)
* فئة [Scenario](/cells/ar/python-net/aspose.cells/scenario)
* فئة [SmartTagCollection](/cells/ar/python-net/aspose.cells.markup/smarttagcollection)
* فئة [TextBox](/cells/ar/python-net/aspose.cells.drawing/textbox)
* فئة [VerticalPageBreakCollection](/cells/ar/python-net/aspose.cells/verticalpagebreakcollection)
