---
title: default_path الملكية
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 40
url: /ar/python-net/aspose.cells/streamprovideroptions/default_path/
is_root: false
---
##  default_path الملكية

المسار الافتراضي (URL) المحفوظ في ملف html الذي تم إنشاؤه للمصدر المشار إليه.
على سبيل المثال ، بيانات الورقة المحفوظة في xxx_files / sheet001.htm ، يجب أن يكون عنوان url المستخدم في ملف html الرئيسي مثل "src =" xxx_files / sheet001.htm ""
###  تعريف:
```python
@property
def default_path(self):
    ...
```

###  أنظر أيضا
* وحدة [aspose.cells](../../)
* فئة [StreamProviderOptions](/cells/ar/python-net/aspose.cells/streamprovideroptions)
