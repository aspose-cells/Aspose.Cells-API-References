---
title: طريقة get_values
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 30
url: /ar/python-net/aspose.cells/referredarea/get_values/
is_root: false
---
##  get_values() {#}
الحصول على قيم الخلية في هذه المنطقة.


###  عائدات

إذا كانت هذه المنطقة غير صالحة ، فسيتم إرجاع "#REF!" ؛
إذا كانت هذه المنطقة عبارة عن خلية واحدة ، فقم بإرجاع كائن قيمة الخلية ؛
وإلا ، فقم بإرجاع صفيف ثنائي الأبعاد لجميع القيم في هذه المنطقة.


```python
def get_values(self):
    ...
```




##  get_values(calculate_formulas) {#bool}
الحصول على قيم الخلية في هذه المنطقة.


###  عائدات

إذا كانت هذه المنطقة غير صالحة ، فسيتم إرجاع "#REF!" ؛
إذا كانت هذه المنطقة عبارة عن خلية واحدة ، فقم بإرجاع كائن قيمة الخلية ؛
وإلا ، فقم بإرجاع صفيف ثنائي الأبعاد لجميع القيم في هذه المنطقة.


```python
def get_values(self, calculate_formulas):
    ...
```


| حدود| يكتب| وصف|
| :- | :- | :- |
| calculate_formulas | bool | في هذا النطاق ، إذا كانت هناك بعض الصيغ التي لم يتم حسابها ،<br/> تشير هذه العلامة إلى ما إذا كان يجب حساب تلك الصيغ بشكل متكرر|



###  أنظر أيضا
* وحدة [aspose.cells](../../)
* فئة [ReferredArea](/cells/ar/python-net/aspose.cells/referredarea)
