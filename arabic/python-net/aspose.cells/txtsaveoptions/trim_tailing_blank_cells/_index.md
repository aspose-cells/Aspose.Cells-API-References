---
title: trim_tailing_blank_cells الملكية
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 230
url: /ar/python-net/aspose.cells/txtsaveoptions/trim_tailing_blank_cells/
is_root: false
---
##  trim_tailing_blank_cells الملكية

يشير إلى ما إذا كان يجب قطع خلايا فارغة في صف واحد أم لا. الافتراضي هو خطأ.

###  ملاحظات

عند الحفظ باستخدام وضع LightCells ولم يتم تحديد [TxtSaveOptions.export_area](/cells/ar/python-net/aspose.cells/txtsaveoptions#export_area) ،
لا يسري هذا الخيار وسيمتد صف واحد فقط إلى الخلية الأخيرة التي يوفرها
تنفيذ [TxtSaveOptions.light_cells_data_provider](/cells/ar/python-net/aspose.cells/txtsaveoptions#light_cells_data_provider)
###  تعريف:
```python
@property
def trim_tailing_blank_cells(self):
    ...
@trim_tailing_blank_cells.setter
def trim_tailing_blank_cells(self, value):
    ...
```

###  أنظر أيضا
* وحدة [aspose.cells](../../)
* فئة [TxtSaveOptions](/cells/ar/python-net/aspose.cells/txtsaveoptions)
