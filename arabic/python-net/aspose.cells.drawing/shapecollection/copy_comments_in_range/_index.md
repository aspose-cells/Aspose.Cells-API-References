---
title: طريقة copy_comments_in_range
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 370
url: /ar/python-net/aspose.cells.drawing/shapecollection/copy_comments_in_range/
is_root: false
---
##  copy_comments_in_range(shapes, ca, dest_row, dest_column) {#ShapeCollection-CellArea-int-int}
انسخ جميع التعليقات في النطاق.



```python
def copy_comments_in_range(self, shapes, ca, dest_row, dest_column):
    ...
```


| معامل| يكتب| وصف|
| :- | :- | :- |
| shapes | [ShapeCollection](/cells/ar/python-net/aspose.cells.drawing/shapecollection) | أشكال المصدر.|
| ca | [CellArea](/cells/ar/python-net/aspose.cells/cellarea) | نطاق المصدر.|
| dest_row | int | صف بداية النطاق الوجهة.|
| dest_column | int | عمود بداية النطاق المقسم.|



###  أنظر أيضا
* وحدة [aspose.cells.drawing](../../)
* فئة [ShapeCollection](/cells/ar/python-net/aspose.cells.drawing/shapecollection)
