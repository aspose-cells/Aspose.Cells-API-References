---
title: end_arrowhead_style الملكية
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 370
url: /ar/python-net/aspose.cells.drawing/lineshape/end_arrowhead_style/
is_root: false
---
##  end_arrowhead_style الملكية

الحصول على نمط رأس سهم النهاية للخط وتعيينه.

###  ملاحظات

 ملاحظة: هذا العضو قد عفا عليه الزمن الآن. بدلاً من،
 الرجاء استخدام خاصية Shape.Line.EndArrowheadStyle.
 ستتم إزالة هذا العقار بعد 12 شهرًا منذ أغسطس 2016.
Aspose يعتذر عن أي إزعاج قد يكون سببه لك.
###  تعريف:
```python
@property
def end_arrowhead_style(self):
    ...
@end_arrowhead_style.setter
def end_arrowhead_style(self, value):
    ...
```

###  أنظر أيضا
* وحدة [aspose.cells.drawing](../../)
* فئة [LineShape](/cells/ar/python-net/aspose.cells.drawing/lineshape)
* فئة [MsoArrowheadStyle](/cells/ar/python-net/aspose.cells.drawing/msoarrowheadstyle)
