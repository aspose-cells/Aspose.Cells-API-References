---
title: has_series_lines الملكية
second_title: Aspose.Cells for Python via .NET API المراجع
description:
type: docs
weight: 240
url: /ar/python-net/aspose.cells.charts/series/has_series_lines/
is_root: false
---
##  has_series_lines الملكية

صواب إذا كان المخطط العمودي المكدس أو المخطط الشريطي يحتوي على خطوط متسلسلة أو
 إذا كان المخطط الدائري أو المخطط الشريطي يحتوي على خطوط موصل بين القسمين.
ينطبق فقط على المخططات العمودية المكدسة أو المخططات الشريطية أو المخططات الدائرية أو المخططات الدائرية الشريطية.
###  تعريف:
```python
@property
def has_series_lines(self):
    ...
@has_series_lines.setter
def has_series_lines(self, value):
    ...
```

###  أنظر أيضا
* وحدة [aspose.cells.charts](../../)
* فئة [Series](/cells/ar/python-net/aspose.cells.charts/series)
