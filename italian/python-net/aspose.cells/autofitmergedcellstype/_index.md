---
title: AutoFitMergedCellsType enumerazione
second_title: Aspose.Cells for Python via .NET API Referenze
description:
type: docs
weight: 1760
url: /it/python-net/aspose.cells/autofitmergedcellstype/
is_root: false
---
##  AutoFitMergedCellsType enumerazione
Rappresenta il tipo di celle unite con adattamento automatico.



Il tipo AutoFitMergedCellsType espone i membri seguenti:

###  Campi
| Campo| Descrizione|
| :- | :- |
| NONE | Ignora le celle unite.|
| FIRST_LINE | Espande solo l'altezza della prima riga.|
| LAST_LINE |Espande solo l'altezza dell'ultima riga.|
| EACH_LINE | Espande solo l'altezza di ogni riga.|



###  Guarda anche
* modulo [aspose.cells](..)
