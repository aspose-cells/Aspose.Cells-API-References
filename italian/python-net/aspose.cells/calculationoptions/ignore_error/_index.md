---
title: ignore_error proprietà
second_title: Aspose.Cells for Python via .NET API Referenze
description:
type: docs
weight: 70
url: /it/python-net/aspose.cells/calculationoptions/ignore_error/
is_root: false
---
##  ignore_error proprietà

Indica se è necessario nascondere l'errore nel calcolo delle formule.
L'errore potrebbe essere dovuto a funzioni non supportate, collegamenti esterni e così via.
###  Definizione:
```python
@property
def ignore_error(self):
    ...
@ignore_error.setter
def ignore_error(self, value):
    ...
```

###  Guarda anche
* modulo [aspose.cells](../../)
* classe [CalculationOptions](/cells/it/python-net/aspose.cells/calculationoptions)
