---
title: metodo get_cell_or_null
second_title: Aspose.Cells for Python via .NET API Referenze
description:
type: docs
weight: 80
url: /it/python-net/aspose.cells/range/get_cell_or_null/
is_root: false
---
##  get_cell_or_null(row_offset, column_offset) {#int-int}
Ottiene [Cell](/cells/it/python-net/aspose.cells/cell) oggetto o null in questo intervallo.


###  ritorna

[Cell](/cells/it/python-net/aspose.cells/cell) oggetto.


```python
def get_cell_or_null(self, row_offset, column_offset):
    ...
```


| Parametro| Tipo| Descrizione|
| :- | :- | :- |
| row_offset | int | Offset riga in questo intervallo, in base zero.|
| column_offset | int |Offset di colonna in questo intervallo, in base zero.|



###  Guarda anche
* modulo [aspose.cells](../../)
* classe [Cell](/cells/it/python-net/aspose.cells/cell)
* classe [Range](/cells/it/python-net/aspose.cells/range)
