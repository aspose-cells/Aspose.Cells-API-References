---
title: metodo end_cell_in_column
second_title: Aspose.Cells for Python via .NET API Referenze
description:
type: docs
weight: 270
url: /it/python-net/aspose.cells/cells/end_cell_in_column/
is_root: false
---
##  end_cell_in_column(column_index) {#int}
Ottiene l'ultima cella in questa colonna.


###  ritorna

Cell oggetto.


```python
def end_cell_in_column(self, column_index):
    ...
```


| Parametro| Tipo| Descrizione|
| :- | :- | :- |
| column_index | int | Indice di colonna.|


##  end_cell_in_column(start_row, end_row, start_column, end_column) {#int-int-int-int}
Ottiene l'ultima cella con l'indice di colonna massimo in questo intervallo.


###  ritorna

Cell oggetto.


```python
def end_cell_in_column(self, start_row, end_row, start_column, end_column):
    ...
```


| Parametro| Tipo| Descrizione|
| :- | :- | :- |
| start_row | int | Inizia l'indice della riga.|
| end_row | int | Indice di fine riga.|
| start_column | int | Inizia l'indice della colonna.|
| end_column | int | Fine dell'indice di colonna.|



###  Guarda anche
* modulo [aspose.cells](../../)
* classe [Cells](/cells/it/python-net/aspose.cells/cells)
