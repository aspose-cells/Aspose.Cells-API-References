---
title: metodo filter
second_title: Aspose.Cells for Python via .NET API Referenze
description:
type: docs
weight: 90
url: /it/python-net/aspose.cells/autofilter/filter/
is_root: false
---
##  filter(field_index, criteria) {#int-str}
Filtra un elenco con i criteri specificati.



```python
def filter(self, field_index, criteria):
    ...
```


| Parametro| Tipo| Descrizione|
| :- | :- | :- |
| field_index | int | L'intero offset del campo su cui si desidera basare il filtro<br/> (da sinistra dell'elenco; il campo più a sinistra è il campo 0).|
| criteria | str | I criteri specificati (una stringa; ad esempio, "101").|
###  Osservazioni

Aspose.Cells rimuoverà tutte le altre impostazioni filter in questo campo come Ms Excel 97-2003.


###  Guarda anche

* modulo [aspose.cells](../../)
* classe [AutoFilter](/cells/it/python-net/aspose.cells/autofilter)
