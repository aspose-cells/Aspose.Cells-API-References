---
title: classe PageEndSavingArgs
second_title: Aspose.Cells for Python via .NET API Referenze
description:
type: docs
weight: 50
url: /it/python-net/aspose.cells.rendering/pageendsavingargs/
is_root: false
---
##  classe PageEndSavingArgs
Le informazioni per una pagina terminano il processo di salvataggio.



**Eredità:** [PageEndSavingArgs](/cells/python-net/aspose.cells.rendering/pageendsavingargs) → 
[PageSavingArgs](/cells/it/python-net/aspose.cells.rendering/pagesavingargs)



Il tipo PageEndSavingArgs espone i membri seguenti:

###  Proprietà
| Proprietà| Descrizione|
| :- | :- |
| [page_index](/cells/it/python-net/aspose.cells.rendering/pageendsavingargs/page_index) | Indice della pagina corrente, in base zero.|
| [page_count](/cells/it/python-net/aspose.cells.rendering/pageendsavingargs/page_count) | Numero totale di pagine.|
| [has_more_pages](/cells/it/python-net/aspose.cells.rendering/pageendsavingargs/has_more_pages) | Ottiene o imposta un valore che indica se devono essere restituite più pagine.<br/> Il valore predefinito è vero.|



###  Guarda anche
* modulo [aspose.cells.rendering](..)
* classe [PageEndSavingArgs](/cells/it/python-net/aspose.cells.rendering/pageendsavingargs)
* classe [PageSavingArgs](/cells/it/python-net/aspose.cells.rendering/pagesavingargs)
