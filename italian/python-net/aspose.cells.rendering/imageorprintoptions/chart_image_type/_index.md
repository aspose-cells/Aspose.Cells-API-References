---
title: chart_image_type proprietà
second_title: Aspose.Cells for Python via .NET API Referenze
description:
type: docs
weight: 50
url: /it/python-net/aspose.cells.rendering/imageorprintoptions/chart_image_type/
is_root: false
---
##  chart_image_type proprietà

Indica il tipo di immagine del grafico durante la conversione.
valore predefinito: PNG.

###  Osservazioni

NOTA: questo membro è ora obsoleto. Invece,
Il grafico e la forma sono sempre resi come elementi vettoriali (ad es. punto, linea) per la qualità del rendering.
 Questa proprietà verrà rimossa 12 mesi dopo da giugno 2022.
Aspose si scusa per gli eventuali disagi causati.
###  Definizione:
```python
@property
def chart_image_type(self):
    ...
@chart_image_type.setter
def chart_image_type(self, value):
    ...
```

###  Guarda anche
* modulo [aspose.cells.rendering](../../)
* classe [ImageOrPrintOptions](/cells/it/python-net/aspose.cells.rendering/imageorprintoptions)
