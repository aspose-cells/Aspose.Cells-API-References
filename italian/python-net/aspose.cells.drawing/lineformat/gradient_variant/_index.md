---
title: gradient_variant proprietà
second_title: Aspose.Cells for Python via .NET API Referenze
description:
type: docs
weight: 220
url: /it/python-net/aspose.cells.drawing/lineformat/gradient_variant/
is_root: false
---
##  gradient_variant proprietà

Restituisce la variante sfumatura per il riempimento specificato.
Si applica solo a Excel 2007.

###  Osservazioni

Può essere solo un valore compreso tra 1 e 4, corrispondente a una delle quattro varianti nella scheda Sfumatura nella finestra di dialogo Effetti di riempimento. Se style è GradientStyle.FromCenter, l'argomento Variant può essere solo 1 o 2.
###  Definizione:
```python
@property
def gradient_variant(self):
    ...
```

###  Guarda anche
* modulo [aspose.cells.drawing](../../)
* classe [LineFormat](/cells/it/python-net/aspose.cells.drawing/lineformat)
