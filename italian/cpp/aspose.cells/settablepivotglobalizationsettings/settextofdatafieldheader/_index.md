---
title: Aspose::Cells::SettablePivotGlobalizationSettings::SetTextOfDataFieldHeader method
linktitle: SetTextOfDataFieldHeader
second_title: Aspose.Cells for C++ API Reference
description: 'Aspose::Cells::SettablePivotGlobalizationSettings::SetTextOfDataFieldHeader method. Sets the the text of the value area field header in the PivotTable in C++.'
type: docs
weight: 2300
url: /it/cpp/aspose.cells/settablepivotglobalizationsettings/settextofdatafieldheader/
---
## SettablePivotGlobalizationSettings::SetTextOfDataFieldHeader(const U16String\&) method


Sets the the text of the value area field header in the PivotTable.

```cpp
void Aspose::Cells::SettablePivotGlobalizationSettings::SetTextOfDataFieldHeader(const U16String &text)
```


| Parameter | Type | Description |
| --- | --- | --- |
| text | const U16String\& | The text of data field header name |

## See Also

* Class [Vector](../../vector/)
* Class [U16String](../../u16string/)
* Class [SettablePivotGlobalizationSettings](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## SettablePivotGlobalizationSettings::SetTextOfDataFieldHeader(const char16_t*) method


Sets the the text of the value area field header in the PivotTable.

```cpp
void Aspose::Cells::SettablePivotGlobalizationSettings::SetTextOfDataFieldHeader(const char16_t *text)
```


| Parameter | Type | Description |
| --- | --- | --- |
| text | const char16_t* | The text of data field header name |

## See Also

* Class [Vector](../../vector/)
* Class [SettablePivotGlobalizationSettings](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
