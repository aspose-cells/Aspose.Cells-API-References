---
title: Aspose::Cells::SettableGlobalizationSettings::SetGrandTotalName method
linktitle: SetGrandTotalName
second_title: Aspose.Cells for C++ API Reference
description: 'Aspose::Cells::SettableGlobalizationSettings::SetGrandTotalName method. Sets the grand total name of specific function in C++.'
type: docs
weight: 900
url: /it/cpp/aspose.cells/settableglobalizationsettings/setgrandtotalname/
---
## SettableGlobalizationSettings::SetGrandTotalName(ConsolidationFunction, const U16String\&) method


Sets the grand total name of specific function.

```cpp
void Aspose::Cells::SettableGlobalizationSettings::SetGrandTotalName(ConsolidationFunction functionType, const U16String &name)
```


| Parameter | Type | Description |
| --- | --- | --- |
| functionType | ConsolidationFunction | The function type. |
| name | const U16String\& | The grand total name of the function. |

## See Also

* Class [Vector](../../vector/)
* Enum [ConsolidationFunction](../../consolidationfunction/)
* Class [U16String](../../u16string/)
* Class [SettableGlobalizationSettings](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## SettableGlobalizationSettings::SetGrandTotalName(ConsolidationFunction, const char16_t*) method


Sets the grand total name of specific function.

```cpp
void Aspose::Cells::SettableGlobalizationSettings::SetGrandTotalName(ConsolidationFunction functionType, const char16_t *name)
```


| Parameter | Type | Description |
| --- | --- | --- |
| functionType | ConsolidationFunction | The function type. |
| name | const char16_t* | The grand total name of the function. |

## See Also

* Class [Vector](../../vector/)
* Enum [ConsolidationFunction](../../consolidationfunction/)
* Class [SettableGlobalizationSettings](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
