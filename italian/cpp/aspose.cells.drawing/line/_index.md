...
type: docs
weight: 2800
url: /it/cpp/aspose.cells.drawing/line/
---
## Line class
...
//Applicare uno stile a linea puntata sulle linee di una NSeries
chart.GetNSeries().Get(0).GetBorder().SetStyle(LineType::Dot);
chart.GetNSeries().Get(0).GetBorder().SetColor(Color{ 0xff, 0xff, 0, 0 });
//Applicare uno stile di marcatore triangolare sui marcatori dati di una NSeries
chart.GetNSeries().Get(0).GetMarker().SetMarkerStyle(ChartMarkerType::Triangle);
//Impostare lo spessore di tutte le linee in una NSeries a medio
