---
title: Aspose::Cells::Drawing::GroupShape class
linktitle: GroupShape
second_title: Aspose.Cells for C++ API Reference
description: 'Aspose::Cells::Drawing::GroupShape class. Represents the group shape which contains the individual shapes in C++.'
type: docs
weight: 2600
url: /it/cpp/aspose.cells.drawing/groupshape/
---
## GroupShape class
...
```cpp
class GroupShape : public Aspose::Cells::Drawing::Shape
```
...
| [Get(int32_t index)](./get/) | Gets the child shape by index. |
...
| [GroupShape(GroupShape_Impl* impl)](./groupshape/) | Constructs from an implementation object. |
| [GroupShape(const GroupShape\& src)](./groupshape/) | Copy constructor. |
| [GroupShape(const Shape\& src)](./groupshape/) | Constructs from a parent object. |
...
| [operator=(const GroupShape\& src)](./operator_asm/) | operator= |
...
| [~GroupShape()](./~groupshape/) | Destructor. |
...
//Rendi il pulsante radio 3-D.
radio1.SetShadow(true);

//Rendi visibile il formato linea.
radio1.GetLine().SetFillType(FillType::Solid);

//Rendi visibile il formato riempimento.
radio1.GetFill().SetFillType(FillType::Solid);

//Imposta il colore di primo piano del pulsante radio.
if(radio1.GetFill().GetFillType() == FillType::Solid)
{
    radio1.GetFill().GetSolidFill().SetColor(Color{ 0xff, 0x90, 0xee, 0x90 });//LightGreen
}

//Imposta lo stile della linea del pulsante radio.
radio1.GetLine().SetCompoundType(MsoLineStyle::ThickThin);

//Imposta lo spessore del pulsante radio.
radio1.GetLine().SetWeight(4);

//Imposta il colore della linea del pulsante radio.
if (radio1.GetLine().GetFillType() == FillType::Solid)
{
    radio1.GetLine().GetSolidFill().SetColor(Color{ 0xff, 0, 0, 0xff });//Blue
}

//Imposta lo stile tratteggiato del pulsante radio.
radio1.GetLine().SetDashStyle(MsoLineDashStyle::Solid);

//Aggiungi un altro pulsante radio.
...
//Imposta il colore di primo piano del pulsante radio.
if (radio2.GetFill().GetFillType() == FillType::Solid)
{
    radio2.GetFill().GetSolidFill().SetColor(Color{ 0xff, 0x90, 0xee, 0x90 });//LightGreen
}
//Imposta lo stile della linea del pulsante radio.
...
//Imposta il colore della linea del pulsante radio.
if (radio2.GetLine().GetFillType() == FillType::Solid)
{
    radio2.GetLine().GetSolidFill().SetColor(Color{ 0xff, 0, 0, 0xff });//Blue
}
//Imposta lo stile tratteggiato del pulsante radio.
...
//Imposta il colore di primo piano del pulsante radio.
if (radio3.GetFill().GetFillType() == FillType::Solid)
{
    radio3.GetFill().GetSolidFill().SetColor(Color{ 0xff, 0x90, 0xee, 0x90 });//LightGreen
}
...
//Imposta il colore della linea del pulsante radio.
if (radio3.GetLine().GetFillType() == FillType::Solid)
{
    radio3.GetLine().GetSolidFill().SetColor(Color{ 0xff, 0, 0, 0xff });//Blue
}
//Imposta lo stile tratteggiato del pulsante radio.
...
//Salva il file Excel.
excelbook.Save(u"groupshapes.xls");
