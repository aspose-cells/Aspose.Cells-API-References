---
title: ControlScrollBarType uppräkning
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 240
url: /sv/python-net/aspose.cells.drawing.activexcontrols/controlscrollbartype/
is_root: false
---
##  ControlScrollBarType uppräkning
Representerar typen av rullningslist.



Typen ControlScrollBarType avslöjar följande medlemmar:

###  Fält
| Fält| Beskrivning|
| :- | :- |
| NONE | Visar inga rullningslister.|
| HORIZONTAL | Visar en horisontell rullningslist.|
| BARS_VERTICAL | Visar en vertikal rullningslist.|
| BARS_BOTH | Visar både en horisontell och en vertikal rullningslist.|



###  Se även
* modul [aspose.cells.drawing.activexcontrols](..)
