---
title: ControlMousePointerType uppräkning
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 200
url: /sv/python-net/aspose.cells.drawing.activexcontrols/controlmousepointertype/
is_root: false
---
##  ControlMousePointerType uppräkning
Representerar typen av ikon som visas som muspekare för kontrollen.



Typen ControlMousePointerType avslöjar följande medlemmar:

###  Fält
| Fält| Beskrivning|
| :- | :- |
| DEFAULT | Standardpekare.|
| ARROW | Pil.|
| CROSS | Hårkorspekare.|
| I_BEAM | Jag strålar.|
| SIZE_NESW | Dubbelpil som pekar nordost och sydväst.|
| SIZE_NS | Dubbelpil som pekar mot norr och söder.|
| SIZE_NWSE | Dubbelpil som pekar nordväst och sydost.|
| SIZE_WE | Dubbelpil som pekar västerut och österut.|
| UP_ARROW | Uppåtpil.|
| HOUR_GLASS | Timglas.|
| NO_DROP | "Inte"-symbol (cirkel med en diagonal linje) ovanpå objektet som dras.|
| APP_STARTING | Pil med ett timglas.|
| HELP | Pil med ett frågetecken.|
| SIZE_ALL |Markören "Size-all" (pilar som pekar mot norr, söder, öster och väster).|
| CUSTOM | Använder ikonen som anges av MouseIcon-egenskapen.|



###  Se även
* modul [aspose.cells.drawing.activexcontrols](..)
