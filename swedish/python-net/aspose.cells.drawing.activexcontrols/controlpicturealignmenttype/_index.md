---
title: ControlPictureAlignmentType uppräkning
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 210
url: /sv/python-net/aspose.cells.drawing.activexcontrols/controlpicturealignmenttype/
is_root: false
---
##  ControlPictureAlignmentType uppräkning
Representerar justeringen av bilden inuti formuläret eller bilden.



Typen ControlPictureAlignmentType avslöjar följande medlemmar:

###  Fält
| Fält| Beskrivning|
| :- | :- |
| TOP_LEFT | Det övre vänstra hörnet.|
| TOP_RIGHT | Det övre högra hörnet.|
| CENTER | Mitten.|
| BOTTOM_LEFT | Nedre vänstra hörnet.|
| BOTTOM_RIGHT | Det nedre högra hörnet.|



###  Se även
* modul [aspose.cells.drawing.activexcontrols](..)
