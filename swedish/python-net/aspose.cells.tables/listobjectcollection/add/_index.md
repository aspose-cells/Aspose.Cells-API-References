---
title: add metod
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 20
url: /sv/python-net/aspose.cells.tables/listobjectcollection/add/
is_root: false
---
##  add(start_cell, end_cell, has_headers) {#str-str-bool}
Lägger till ett ListObject till kalkylbladet.


###  Returnerar

Indexet för det nya ListObject


```python
def add(self, start_cell, end_cell, has_headers):
    ...
```


| Parametrar| Typ| Beskrivning|
| :- | :- | :- |
| start_cell | str | Startcellen i listintervallet.|
| end_cell | str | Slutcellen för listintervallet.|
| has_headers | bool | Om intervallet har rubriker.|


##  add(start_row, start_column, end_row, end_column, has_headers) {#int-int-int-int-bool}
Lägger till ett ListObject till kalkylbladet.


###  Returnerar

Indexet för det nya ListObject


```python
def add(self, start_row, start_column, end_row, end_column, has_headers):
    ...
```


| Parametrar| Typ| Beskrivning|
| :- | :- | :- |
| start_row | int | Startraden i listintervallet.|
| start_column | int | Startraden i listintervallet.|
| end_row | int | Startraden i listintervallet.|
| end_column | int | Startraden i listintervallet.|
| has_headers | bool | Om intervallet har rubriker.|



###  Se även
* modul [aspose.cells.tables](../../)
* klass [ListObjectCollection](/cells/sv/python-net/aspose.cells.tables/listobjectcollection)
