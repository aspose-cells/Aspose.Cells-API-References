---
title: OdsPageBackgroundGraphicPositionType uppräkning
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 60
url: /sv/python-net/aspose.cells.ods/odspagebackgroundgraphicpositiontype/
is_root: false
---
##  OdsPageBackgroundGraphicPositionType uppräkning
Representerar positionen.



Typen OdsPageBackgroundGraphicPositionType avslöjar följande medlemmar:

###  Fält
| Fält| Beskrivning|
| :- | :- |
| TOP_LEFT | Övre vänstra.|
| TOP_CENTER | Översta mitten.|
| TOP_RIGHT | Överst till höger.|
| CENTER_LEFT |Mitten vänster.|
| CENTER_CENTER | Centrum.|
| CENTER_RIGHT | Mitten höger.|
| BOTTOM_LEFT | Nedre vänstra.|
| BOTTOM_CENTER | Nedre mitten.|
| BOTTOM_RIGHT | Nere till höger.|



###  Se även
* modul [aspose.cells.ods](..)
