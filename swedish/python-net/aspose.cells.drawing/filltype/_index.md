---
title: FillType uppräkning
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 830
url: /sv/python-net/aspose.cells.drawing/filltype/
is_root: false
---
##  FillType uppräkning
Fyllningsformattyp.



Typen FillType avslöjar följande medlemmar:

###  Fält
| Fält| Beskrivning|
| :- | :- |
| AUTOMATIC | Representerar automatisk formateringstyp.|
| NONE | Representerar ingen formateringstyp.|
| SOLID | Fast fyllningsformat.|
| GRADIENT | Gradientfyllningsformat.|
| TEXTURE | Texturfyllningsformat (inkluderar bildfyllning).|
| PATTERN | Mönsterfyllningsformat.|
| GROUP | Ärv gruppens fyllningsegenskaper.|



###  Se även
* modul [aspose.cells.drawing](..)
