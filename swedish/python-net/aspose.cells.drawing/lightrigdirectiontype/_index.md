---
title: LightRigDirectionType uppräkning
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 910
url: /sv/python-net/aspose.cells.drawing/lightrigdirectiontype/
is_root: false
---
##  LightRigDirectionType uppräkning
Representerar typen av ljusriggriktning.



Typen LightRigDirectionType avslöjar följande medlemmar:

###  Fält
| Fält| Beskrivning|
| :- | :- |
| BOTTOM | Botten|
| BOTTOM_LEFT | Nedre vänstra.|
| BOTTOM_RIGHT | Nere till höger.|
| LEFT | Vänster.|
| RIGHT | Höger.|
| TOP | Topp.|
| TOP_LEFT | Övre vänstra.|
| TOP_RIGHT | Överst till höger.|



###  Se även
* modul [aspose.cells.drawing](..)
