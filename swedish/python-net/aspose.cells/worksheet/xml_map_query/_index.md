---
title: xml_map_query metod
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 310
url: /sv/python-net/aspose.cells/worksheet/xml_map_query/
is_root: false
---
##  xml_map_query(path, xml_map) {#str-XmlMap}
Fråga cellområden som mappade/länkade till den specifika sökvägen för xml-kartan.


###  Returnerar

[CellArea](/cells/sv/python-net/aspose.cells/cellarea) lista som mappade/länkade till den specifika sökvägen till xml-kartan, returneras en tom lista om inget är mappat/länkat.


```python
def xml_map_query(self, path, xml_map):
    ...
```


| Parametrar| Typ| Beskrivning|
| :- | :- | :- |
| path | str | xml-elementets sökväg|
| xml_map | [XmlMap](/cells/sv/python-net/aspose.cells/xmlmap) |Ange en xml-karta om du vill fråga efter den specifika sökvägen inom en specifik karta|



###  Se även
* modul [aspose.cells](../../)
* klass [CellArea](/cells/sv/python-net/aspose.cells/cellarea)
* klass [Worksheet](/cells/sv/python-net/aspose.cells/worksheet)
