---
title: OoxmlCompressionType uppräkning
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 2280
url: /sv/python-net/aspose.cells/ooxmlcompressiontype/
is_root: false
---
##  OoxmlCompressionType uppräkning
Kompressionstypen Ooxml



Typen OoxmlCompressionType avslöjar följande medlemmar:

###  Fält
| Fält| Beskrivning|
| :- | :- |
| LEVEL1 | Den snabbaste men minst effektiva kompressionen.|
| LEVEL2 | Lite långsammare, men bättre, än nivå 1.|
| LEVEL3 | Lite långsammare, men bättre, än nivå 2.|
| LEVEL4 | Lite långsammare, men bättre, än nivå 3.|
| LEVEL5 | Lite långsammare än nivå 4, men med bättre kompression.|
| LEVEL6 | En bra balans mellan hastighet och kompressionseffektivitet.|
| LEVEL7 | Ganska bra kompression!|
| LEVEL8 | Bättre komprimering än Level7!|
| LEVEL9 |Den "bästa" komprimeringen, där bäst betyder största minskningen av storleken på indataströmmen.<br/> Detta är också den långsammaste kompressionen.|



###  Se även
* modul [aspose.cells](..)
