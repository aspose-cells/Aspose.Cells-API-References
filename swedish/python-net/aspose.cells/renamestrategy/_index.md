---
title: RenameStrategy uppräkning
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 2430
url: /sv/python-net/aspose.cells/renamestrategy/
is_root: false
---
##  RenameStrategy uppräkning
Strategialternativ för duplicerade namn på kolumner.



Typen RenameStrategy avslöjar följande medlemmar:

###  Fält
| Fält| Beskrivning|
| :- | :- |
| EXCEPTION | Kastar undantag.|
| DIGIT | Namngiven med siffra. Duplicerade namn blir ...1, ...2 osv.|
| LETTER | Namngivna med bokstav.. Duplicerade namn blir ...A, ...B osv.|



###  Anmärkningar

Vid bearbetning av data med rubriker kräver vissa scenarier att rubrikerna inte är duplicerade för alla kolumner.
Till exempel, när du exporterar data till en datatabell och rubriken måste tas som datatabellens kolumnnamn,
dubblerade värden i rubriken är ogiltiga.
För sådana situationer kan användaren bestämma hur de ska hanteras genom att specificera denna strategi.

###  Se även
* modul [aspose.cells](..)
