---
title: parsing_formula_on_open fastighet
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 450
url: /sv/python-net/aspose.cells/workbooksettings/parsing_formula_on_open/
is_root: false
---
##  parsing_formula_on_open fastighet

Anger om formeln analyseras när filen läses.

###  Anmärkningar

Gäller endast Excel Xlsx-, Xltx-, Xltm- och Xlsm-filer
eftersom formlerna i filerna lagras med en strängformel.
OBS: Den här medlemmen är nu föråldrad. Använd istället LoadOptions.ParsingFormulaOnOpen.
 Den här egenskapen kommer att tas bort 12 månader senare sedan januari 2020.
Aspose ber om ursäkt för eventuella besvär du kan ha upplevt.
###  Definition:
```python
@property
def parsing_formula_on_open(self):
    ...
@parsing_formula_on_open.setter
def parsing_formula_on_open(self, value):
    ...
```

###  Se även
* modul [aspose.cells](../../)
* klass [WorkbookSettings](/cells/sv/python-net/aspose.cells/workbooksettings)
