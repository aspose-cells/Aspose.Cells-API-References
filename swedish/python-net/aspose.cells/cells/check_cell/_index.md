---
title: check_cell metod
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 60
url: /sv/python-net/aspose.cells/cells/check_cell/
is_root: false
---
##  check_cell(row, column) {#int-int}
Hämtar elementet [Cell](/cells/sv/python-net/aspose.cells/cell) eller null vid angivet cellradindex och kolumnindex.


###  Returnerar

Returnera Cell-objekt om ett Cell-objekt finns.
Returnera null om cellen inte finns.


```python
def check_cell(self, row, column):
    ...
```


| Parameter| Typ| Beskrivning|
| :- | :- | :- |
| row | int | Radindex|
| column | int | Kolumnindex|



###  Se även
* modul [aspose.cells](../../)
* klass [Cell](/cells/sv/python-net/aspose.cells/cell)
* klass [Cells](/cells/sv/python-net/aspose.cells/cells)
