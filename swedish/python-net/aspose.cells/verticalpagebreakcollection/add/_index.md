---
title: add metod
second_title: Aspose.Cells for Python via .NET API Referenser
description:
type: docs
weight: 20
url: /sv/python-net/aspose.cells/verticalpagebreakcollection/add/
is_root: false
---
##  add(column) {#int}
Lägger till en vertikal sidbrytning i samlingen.


###  Returnerar

[VerticalPageBreak](/cells/sv/python-net/aspose.cells/verticalpagebreak) objektindex.


```python
def add(self, column):
    ...
```


| Parametrar| Typ| Beskrivning|
| :- | :- | :- |
| column | int | Cell kolumnindex, nollbaserat.|
###  Anmärkningar

Sidbrytning läggs till längst upp till vänster i cellen.
Ställ in en horisontell sidbrytning och en vertikal sidbrytning samtidigt.

##  add(cell_name) {#str}
Lägger till en vertikal sidbrytning i samlingen.


###  Returnerar

[VerticalPageBreak](/cells/sv/python-net/aspose.cells/verticalpagebreak) objektindex.


```python
def add(self, cell_name):
    ...
```


| Parametrar| Typ| Beskrivning|
| :- | :- | :- |
| cell_name | str | Cell namn.|
###  Anmärkningar

Sidbrytning läggs till längst upp till vänster i cellen.
Ställ in en horisontell sidbrytning och en vertikal sidbrytning samtidigt.

##  add(row, column) {#int-int}
Lägger till en vertikal sidbrytning i samlingen.


###  Returnerar

[VerticalPageBreak](/cells/sv/python-net/aspose.cells/verticalpagebreak) objektindex.


```python
def add(self, row, column):
    ...
```


| Parametrar| Typ| Beskrivning|
| :- | :- | :- |
| row | int | Cell radindex, nollbaserat.|
| column | int | Cell kolumnindex, nollbaserat.|
###  Anmärkningar

Sidbrytning läggs till längst upp till vänster i cellen.
Ställ in en horisontell sidbrytning och en vertikal sidbrytning samtidigt.

##  add(start_row, end_row, column) {#int-int-int}
Lägger till en vertikal sidbrytning i samlingen.


###  Returnerar

[VerticalPageBreak](/cells/sv/python-net/aspose.cells/verticalpagebreak) objektindex.


```python
def add(self, start_row, end_row, column):
    ...
```


| Parametrar| Typ| Beskrivning|
| :- | :- | :- |
| start_row | int | Startradindex, nollbaserat.|
| end_row | int |Slutradindex, nollbaserat.|
| column | int | Kolumnindex, nollbaserat.|
###  Anmärkningar

Den här metoden används för att add göra en vertikal sidbrytning inom ett utskriftsområde.


###  Se även

* modul [aspose.cells](../../)
* klass [VerticalPageBreak](/cells/sv/python-net/aspose.cells/verticalpagebreak)
* klass [VerticalPageBreakCollection](/cells/sv/python-net/aspose.cells/verticalpagebreakcollection)
