---
title: ControlScrollBarType enumeración
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 240
url: /es/python-net/aspose.cells.drawing.activexcontrols/controlscrollbartype/
is_root: false
---
##  ControlScrollBarType enumeración
Representa el tipo de barra de desplazamiento.



El tipo ControlScrollBarType expone los siguientes miembros:

###  Campos
| Campo| Descripción|
| :- | :- |
| NONE | No muestra barras de desplazamiento.|
| HORIZONTAL |Muestra una barra de desplazamiento horizontal.|
| BARS_VERTICAL | Muestra una barra de desplazamiento vertical.|
| BARS_BOTH | Muestra una barra de desplazamiento horizontal y vertical.|



###  Ver también
* módulo [aspose.cells.drawing.activexcontrols](..)
