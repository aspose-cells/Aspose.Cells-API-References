---
title: ShapePathType enumeración
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 1160
url: /es/python-net/aspose.cells.drawing/shapepathtype/
is_root: false
---
##  ShapePathType enumeración
Representa el tipo de segmento de ruta.



El tipo ShapePathType expone los siguientes miembros:

###  Campos
| Campo| Descripción|
| :- | :- |
| LINE_TO |Segmento de línea recta|
| CUBIC_BEZIER_CURVE_TO | Curva de Bézier cúbica|
| MOVE_TO | Iniciar un nuevo camino|
| CLOSE | Si el PUNTO inicial y el PUNTO final no son iguales, un solo<br/> se dibuja una línea recta para conectar el PUNTO inicial y el PUNTO final de la ruta.|
| END | El final del camino actual.|
| ESCAPE | Escapar|
| ARC_TO | Un arco|
| UNKNOWN | Desconocido|



###  Ver también
* módulo [aspose.cells.drawing](..)
