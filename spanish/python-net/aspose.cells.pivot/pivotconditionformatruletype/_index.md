---
title: PivotConditionFormatRuleType enumeración
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 130
url: /es/python-net/aspose.cells.pivot/pivotconditionformatruletype/
is_root: false
---
##  PivotConditionFormatRuleType enumeración
Representa el tipo de regla de formato de condición de tabla dinámica.



El tipo PivotConditionFormatRuleType expone los siguientes miembros:

###  Campos
| Campo| Descripción|
| :- | :- |
| NONE | Indica que el formato condicional Top N no se evalúa|
| ALL | Indica que el formato condicional Top N es<br/> evaluado en todo el rango de alcance.|
| ROW | Indica que el formato condicional Top N se evalúa para cada fila.|
| COLUMN | Indica que el formato condicional Top N es<br/> evaluado para cada columna.|



###  Ver también
* módulo [aspose.cells.pivot](..)
