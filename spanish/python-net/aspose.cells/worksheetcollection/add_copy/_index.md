---
title: add_copy método
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 30
url: /es/python-net/aspose.cells/worksheetcollection/add_copy/
is_root: false
---
##  add_copy(sheet_name) {#str}
Agrega una hoja de trabajo a la colección y copia los datos de una hoja de trabajo existente.


###  Devoluciones

[Worksheet](/cells/es/python-net/aspose.cells/worksheet) índice de objetos.


```python
def add_copy(self, sheet_name):
    ...
```


| Parámetro| Tipo| Descripción|
| :- | :- | :- |
| sheet_name | str | Nombre de la hoja de trabajo fuente.|
###  Excepciones
| Excepción| Descripción|
| :- | :- |
| [CellsException](/cells/es/python-net/aspose.cells/cellsexception) | Especifica un nombre de hoja de cálculo no válido.|




##  add_copy(sheet_index) {#int}
Agrega una hoja de trabajo a la colección y copia los datos de una hoja de trabajo existente.


###  Devoluciones

[Worksheet](/cells/es/python-net/aspose.cells/worksheet) índice de objetos.


```python
def add_copy(self, sheet_index):
    ...
```


| Parámetro| Tipo| Descripción|
| :- | :- | :- |
| sheet_index | int | Índice de la hoja de trabajo fuente.|



###  Ver también
* módulo [aspose.cells](../../)
* clase [CellsException](/cells/es/python-net/aspose.cells/cellsexception)
* clase [Worksheet](/cells/es/python-net/aspose.cells/worksheet)
* clase [WorksheetCollection](/cells/es/python-net/aspose.cells/worksheetcollection)
