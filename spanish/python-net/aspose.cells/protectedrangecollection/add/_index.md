---
title: add método
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 20
url: /es/python-net/aspose.cells/protectedrangecollection/add/
is_root: false
---
##  add(name, start_row, start_column, end_row, end_column) {#str-int-int-int-int}
Agrega un artículo [ProtectedRange](/cells/es/python-net/aspose.cells/protectedrange) a la colección.


###  Devoluciones

índice de objetos


```python
def add(self, name, start_row, start_column, end_row, end_column):
    ...
```


| Parámetro| Tipo| Descripción|
| :- | :- | :- |
| name | str | Título de rango. Esto se usa como un descriptor, no como una definición de rango con nombre.|
| start_row | int | Índice de fila de inicio del rango.|
| start_column | int | Índice de columna de inicio del rango.|
| end_row | int | Índice de fila final del rango.|
| end_column | int | Índice de columna final del rango.|



###  Ver también
* módulo [aspose.cells](../../)
* clase [ProtectedRange](/cells/es/python-net/aspose.cells/protectedrange)
* clase [ProtectedRangeCollection](/cells/es/python-net/aspose.cells/protectedrangecollection)
