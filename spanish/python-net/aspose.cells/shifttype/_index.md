---
title: ShiftType enumeración
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 2470
url: /es/python-net/aspose.cells/shifttype/
is_root: false
---
##  ShiftType enumeración
Representa las opciones de desplazamiento al eliminar un rango de celdas.



El tipo ShiftType expone los siguientes miembros:

###  Campos
| Campo| Descripción|
| :- | :- |
| DOWN | Desplazar celdas hacia abajo.|
| LEFT | Desplazar celdas a la izquierda.|
| NONE | No cambie las celdas.|
| RIGHT | Desplazar celdas a la derecha.|
| UP | Desplace las celdas hacia arriba.|



###  Ver también
* módulo [aspose.cells](..)
