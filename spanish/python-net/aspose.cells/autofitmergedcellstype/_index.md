---
title: AutoFitMergedCellsType enumeración
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 1760
url: /es/python-net/aspose.cells/autofitmergedcellstype/
is_root: false
---
##  AutoFitMergedCellsType enumeración
Representa el tipo de celdas combinadas de ajuste automático.



El tipo AutoFitMergedCellsType expone los siguientes miembros:

###  Campos
| Campo| Descripción|
| :- | :- |
| NONE | Ignorar las celdas combinadas.|
| FIRST_LINE | Solo expande la altura de la primera fila.|
| LAST_LINE |Solo expande la altura de la última fila.|
| EACH_LINE | Solo expande la altura de cada fila.|



###  Ver también
* módulo [aspose.cells](..)
