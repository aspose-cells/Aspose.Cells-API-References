---
title: HtmlLinkTargetType enumeración
second_title: Aspose.Cells for Python via .NET API Referencias
description:
type: docs
weight: 2180
url: /es/python-net/aspose.cells/htmllinktargettype/
is_root: false
---
##  HtmlLinkTargetType enumeración
Representa el tipo de atributo de destino en la etiqueta HTML []().



El tipo HtmlLinkTargetType expone los siguientes miembros:

###  Campos
| Campo| Descripción|
| :- | :- |
| BLANK |Abre el documento vinculado en una nueva ventana o pestaña|
| PARENT | Abre el documento vinculado en el marco principal.|
| SELF | Abre el documento vinculado en el mismo marco en el que se hizo clic (esto es predeterminado)|
| TOP | Abre el documento vinculado en el cuerpo completo de la ventana.|



###  Ver también
* módulo [aspose.cells](..)
