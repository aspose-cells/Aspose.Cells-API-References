---
title: Aspose::Cells::ThreadedCommentAuthorCollection::Get method
linktitle: Get
second_title: Aspose.Cells for C++ API Reference
description: 'Aspose::Cells::ThreadedCommentAuthorCollection::Get method. Gets the person who create threaded comments in C++.'
type: docs
weight: 900
url: /es/cpp/aspose.cells/threadedcommentauthorcollection/get/
---
## ThreadedCommentAuthorCollection::Get(int32_t) method


Gets the person who create threaded comments.

```cpp
ThreadedCommentAuthor Aspose::Cells::ThreadedCommentAuthorCollection::Get(int32_t index)
```


| Parameter | Type | Description |
| --- | --- | --- |
| index | int32_t | The index |

## ReturnValue



## See Also

* Class [ThreadedCommentAuthor](../../threadedcommentauthor/)
* Class [Vector](../../vector/)
* Class [ThreadedCommentAuthorCollection](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## ThreadedCommentAuthorCollection::Get(const U16String\&) method


Gets the person who create threaded comments.

```cpp
ThreadedCommentAuthor Aspose::Cells::ThreadedCommentAuthorCollection::Get(const U16String &name)
```


| Parameter | Type | Description |
| --- | --- | --- |
| name | const U16String\& | The name of the author. |

## ReturnValue



## See Also

* Class [ThreadedCommentAuthor](../../threadedcommentauthor/)
* Class [Vector](../../vector/)
* Class [U16String](../../u16string/)
* Class [ThreadedCommentAuthorCollection](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
## ThreadedCommentAuthorCollection::Get(const char16_t*) method


Gets the person who create threaded comments.

```cpp
ThreadedCommentAuthor Aspose::Cells::ThreadedCommentAuthorCollection::Get(const char16_t *name)
```


| Parameter | Type | Description |
| --- | --- | --- |
| name | const char16_t* | The name of the author. |

## ReturnValue



## See Also

* Class [ThreadedCommentAuthor](../../threadedcommentauthor/)
* Class [Vector](../../vector/)
* Class [ThreadedCommentAuthorCollection](../)
* Namespace [Aspose::Cells](../../)
* Library [Aspose.Cells for C++](../../../)
